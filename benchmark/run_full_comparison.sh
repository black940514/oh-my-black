...
while [[ $# -gt 0 ]]; do
    case $1 in
        ...
        -h|--help)
            ...
            echo ""
            echo "Run complete benchmark comparison between vanilla and OMB modes."
            echo ""
            echo "Options:"
            ...
            echo "  --skip-omc        Skip OMB benchmark run"
...
# Step 2: Run OMB benchmark
if [ "$SKIP_OMB" = false ]; then
    log_step "Step 2/4: Running OMB-enhanced benchmark..."
    echo ""
    "$SCRIPT_DIR/run_omc.sh" $ARGS
    if [ $? -ne 0 ]; then
        log_error "OMB benchmark failed. Aborting."
        exit 1
    fi
    echo ""
else
    log_warn "Skipping OMB benchmark (--skip-omc)"
    echo ""
fi
...
if [ "$SKIP_EVAL" = false ]; then
    ...
    log_step "Step 4/4: Evaluating OMB predictions..."
    echo ""
    if [ -f "$SCRIPT_DIR/evaluate.py" ]; then
        python3 "$SCRIPT_DIR/evaluate.py" \
        ...
        if [ $? -ne 0 ]; then
            log_warn "OMB evaluation had issues (continuing...)"
        fi
    else
        log_warn "evaluate.py not found, skipping evaluation"
    fi
...
log_info "  OMB predictions:     $SCRIPT_DIR/predictions/omc/"
