...
Automated benchmark comparison between vanilla Claude Code and OMB-enhanced Claude Code.
...
- Tests both vanilla and OMB modes
...
- Standard Claude Code without OMB
...
### run_omc.sh
Run OMB-enhanced benchmark:
- Claude Code with oh-my-black orchestration
...
- `--skip-omc` - Skip OMB benchmark run
...
./run_full_comparison.sh --skip-vanilla     # Only run OMB (reuse vanilla results)
```
...
│   └── omc_*.log            # OMB run logs
├── results/
│   ├── vanilla_results.json  # Vanilla evaluation
│   ├── omc_results.json      # OMB evaluation
...
2. **Parallel Runs**: Don't run vanilla and OMB in parallel (share API rate limits)
