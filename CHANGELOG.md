...
- **Project-Scoped Plugin Isolation** (#315, fixes #314) - Fixed project-scoped plugin installations incorrectly modifying global scope. When installing OMB as a project-scoped plugin (in `.claude/plugins/`), the installer was incorrectly modifying global `~/.claude/settings.json` and `~/.claude/hud/`, causing the statusline and plugin to appear in all projects:
...
- **Bash History Integration** (Issue #290) - Claude Code bash commands are now automatically appended to `~/.bash_history`, allowing users to recall them with arrow keys or Ctrl+R in their terminal. Enabled by default; disable with `"bashHistory": false` in `~/.claude/.omb-config.json`.
...
- **Framework-Agnostic Prompts** - All agent prompts, skill files, and TypeScript source code now use generic, language-neutral commands instead of hardcoded npm/Node.js-specific commands. This makes OMB work seamlessly with any project type (Python, Go, Rust, Java, etc.).
...
- **MCP Tool Name Length** (PR #252, fixes #241, #232, #235) - Shortened MCP server name from `omb-tools` to `t` to fix tool names exceeding the 64-character API limit. The longest tool name (`lsp_diagnostics_directory`) now uses 57 characters instead of 65.
...
- **Local-Only State Management** - All execution mode state (ralph, ultrawork, ecomode) is now stored exclusively in `.omb/state/` per project/worktree. Global state in `~/.claude/` and `~/.omb/state/` is no longer written for mode state, enabling multiple git worktrees to run OMB simultaneously without state conflicts. Existing global state is migrated on first read.
- **Promise Pattern Removal** - Fully removed `<promise>` completion pattern from ralph and verification workflows. Completion now uses architect verification + `/oh-my-black:cancel` for clean exit.
...
- **Bun Package Manager Support** (PR #219) - OMB setup now prefers Bun over npm when available, with automatic fallback. Includes duplicate cleanup logic and per-manager verification.
...
Standalone MCP server for Claude Code plugin discovery, making omb-tools visible in the `/mcp` management UI.
...
- **Plugin MCP Configuration** (`.mcp.json`)
  - Declares `omb-tools` server for plugin-scoped discovery
...
- **OMB Tools Server** (`src/mcp/omb-tools-server.ts`)
  - Uses `createSdkMcpServer` and `tool` helpers from `@anthropic-ai/claude-agent-sdk`
  - Exposes tools in MCP format as `mcp__omb-tools__<tool_name>`
...
- `src/mcp/omb-tools-server.ts` - SDK MCP server implementation
- `src/__tests__/omb-tools-server.test.ts` - 10 tests for tool exposure
...
A comprehensive compatibility layer enabling OMB to discover, register, and use external plugins, MCP servers, and tools. This makes OMB a good citizen in the Claude ecosystem by resolving inter-plugin conflicts.
...
- **fix(subagent-tracker):** Replace CPU busy-wait loops with syncSleep (#159)
  ...
  - Complete state path migration to `.omb/state/` across all hooks and templates
...
- **fix(omb-setup):** Use cross-platform date conversion for state age check (#151)
...
- **Smart Agent Suggestions**
  ...
  - Audit logging to `.omb/logs/delegation-audit.jsonl`
...
- Local: `.omb/config.json` → `delegationEnforcementLevel`
- Global: `~/.claude/.omb-config.json` → `delegationEnforcementLevel`

## [3.6.3] - 2026-01-27

### Fixed

- Fix npm package references (`oh-my-black` → `oh-my-black`) in docs/MIGRATION.md, commands/doctor.md, skills/doctor/SKILL.md
...
- **PSM Skill** (`skills/psm/`)
  - `/oh-my-black:psm` command for project session management
  ...
  - Integration with existing oh-my-black workflows
...
All execution mode state files consolidated into `.omb/state/` subdirectory:
...
- **Backfill Deduplication** (`src/analytics/backfill-dedup.ts`)
  ...
  - Persistent state in `~/.omb/state/backfill-state.json`
...
- **Global Token Tracking**
  - All token data stored globally in `~/.omb/state/` (not per-project)
...
- **Website Repository**:
  - Updated oh-my-black-website to v3.4.0
...
- **Ultrapilot**: Parallel autopilot with up to 5 concurrent workers
  ...
  - State tracking in `.omb/state/ultrapilot-state.json` and `.omb/state/ultrapilot-ownership.json`
...
- **Ecomode**: Token-efficient parallel execution mode
  ...
  - State tracking in `.omb/ecomode-state.json`
...
- **State Management Module** (`src/features/state-manager/`)
  - Standardized state locations: `.omb/state/{name}.json` (local), `~/.omb/state/{name}.json` (global)
...
- **Delegation Enforcer Middleware**
  ...
  - Debug mode warnings when `OMB_DEBUG=true`
...
- **Setup Skills**: Consolidated into omb-setup
...
- **omb-setup**: GitHub star prompt after setup completion (#82)
...
- MCP server configuration skill (`/oh-my-black:mcp-setup`) for Context7, Exa, Filesystem, GitHub (#74)
- MCP setup integrated into omb-setup wizard
...
- **Research Command** (`/oh-my-black:research`): Orchestrate parallel scientist agents for complex research
...
- **PreToolUse hooks**: Soft enforcement for delegation via `pre-tool-use.sh` and `pre-tool-use.mjs`
  ...
  - Path-based exceptions for `.omb/`, `.claude/`, `CLAUDE.md`, `AGENTS.md`
...
- **Autopilot Command** (`/oh-my-black:autopilot`): Full autonomous execution from idea to working code
...
- **Cancel-autopilot Skill** (`/oh-my-black:cancel-autopilot`): Graceful cancellation with state preservation
...
- **Update notification**: omb-setup now checks for and notifies about available updates
...
- **omb-setup rebuild behavior**: Now shows version changes when refreshing CLAUDE.md
- **Cache clearing**: omb-setup automatically removes stale plugin cache versions
...
- **Fully-qualified command names**: Documentation now uses `/oh-my-black:omb-setup` and `/oh-my-black:help` for namespace consistency
- **Commands folder populated**: All user-facing commands now have full content (copied from skills)
- **Separated commands vs skills**:
  - 20 commands (user-facing): analyze, cancel-ralph, cancel-ultraqa, cancel-ultrawork, deepinit, deepsearch, doctor, help, hud, learner, note, omb-setup, plan, ralph, ralph-init, ralplan, release, review, ultraqa, ultrawork
  - 6 skills-only (internal): orchestrate, frontend-ui-ux, git-master, omc-default, omc-default-global, planner
- **Consolidated planner into plan**: `planner` is now skill-only; users invoke via `/oh-my-black:plan`
...
- **Project Renamed**: Project renamed to `oh-my-black`
  - npm package remains `oh-my-black`: `npx oh-my-black install`
  - Plugin name is `oh-my-black` for Claude Code integration
...
- **Directory Structure**: `.sisyphus/` → `.omb/` (oh-my-black)
  - State files now in `~/.claude/.omb/`
  - Runtime plans in `.omb/plans/`
  - Session notes in `.omb/notepads/`

- **Environment Variables**: `SISYPHUS_*` → `OMB_*`
...
- **Slash Commands Updated**: Agent-referencing commands now use new names
  - `/oh-my-black:plan` now uses `planner` agent (was `prometheus`)
  - `/oh-my-black:review` now uses `critic` agent (was `momus`)
  - `/oh-my-black:mnemosyne` → `/oh-my-black:learner` for skill extraction

### Migration Guide

For existing users upgrading from 2.x:

1. **Reinstall**: Run `npx oh-my-black install` to update hooks and configs
2. **State Migration**: Old `.sisyphus/` directories will continue to work, but new state saves to `.omb/`
3. **Agent References**: Update any custom scripts/configs that referenced old agent names
4. **Environment Variables**: Rename any `SISYPHUS_*` variables to `OMB_*`
...
- **Mnemosyne - Learned Skills** (`src/hooks/mnemosyne/`)
  - `/oh-my-black:mnemosyne` command to extract reusable skills from conversations
...
- **Sisyphus HUD Statusline** (`src/features/hud/`)
  ...
  - `/oh-my-black:hud` command to configure display options
  - Auto-refresh every ~300ms during active sessions
  - Type-coded agent visualization with model tier colors

- **New Commands**
  - `/oh-my-black:mnemosyne` - Extract learned skills from current conversation
  - `/oh-my-black:hud [preset]` - Configure HUD display (minimal/focused/full)
  - `/oh-my-black:hud status` - Show detailed HUD status
...
- Renamed `/oh-my-black:claudeception` to `/oh-my-black:mnemosyne`
...
- **Help Command** (`commands/help.md`)
  - Comprehensive `/oh-my-black:help` command to guide users on plugin usage
...
- **New Commands**
  - `/oh-my-black:ralph-init <task>` - Scaffold a PRD from task description with auto-generated user stories
  - `/oh-my-black:ultrawork-ralph <task>` - Maximum intensity mode with completion guarantee (ultrawork + ralph loop)
  - `/oh-my-black:ultraqa <goal>` - Autonomous QA cycling workflow (test → verify → fix → repeat)
  - `/oh-my-black:sisyphus-default` - Configure Sisyphus in local project `.claude/CLAUDE.md`
  - `/oh-my-black:sisyphus-default-global` - Configure Sisyphus globally in `~/.claude/CLAUDE.md`
  - `/oh-my-black:note <content>` - Save notes to notepad.md for compaction resilience
...
- **Ralph Loop Enhanced**
  - Auto-initializes PRD when user runs `/oh-my-black:ralph-loop` without existing `prd.json`
...
- **Agent name prefixing** - Standardized all Task() calls to use `oh-my-black:` prefix
...
- **Merged sisyphus+orchestrator+ultrawork into default mode** - 80% behavior overlap consolidated
  ...
  - `/oh-my-black:orchestrator` command deprecated (use default mode or `/oh-my-black:ultrawork`)
...
[2.6.0]: https://github.com/black940514/oh-my-black/compare/v2.5.0...v2.6.0
[2.0.1]: https://github.com/black940514/oh-my-black/compare/v2.0.0...v2.0.1
[1.11.0]: https://github.com/black940514/oh-my-black/compare/v1.10.0...v1.11.0
[1.10.0]: https://github.com/black940514/oh-my-black/compare/v1.9.0...v1.10.0
[1.9.0]: https://github.com/black940514/oh-my-black/compare/v1.8.0...v1.9.0
[1.8.0]: https://github.com/black940514/oh-my-black/compare/v1.7.0...v1.8.0
[1.7.0]: https://github.com/black940514/oh-my-black/releases/tag/v1.7.0
