 ...
 *
 * Integrates external plugins and MCP tools with OMB's permission system.
 * Provides:
...
/**
 * Check if tool should be delegated (not handled directly by OMB)
 */
export function shouldDelegate(toolName: string): boolean {
  ...
  if ((tool.type === 'skill' || tool.type === 'agent') && tool.source !== 'oh-my-black') {
    return true;
  }
...
/**
 * Integrate permission adapter with OMB's permission system
