#!/usr/bin/env node
/**
 * OMB HUD - Main Entry Point
 *
 * Statusline command that visualizes oh-my-black state.
...
async function calculateSessionHealth(
  ...
  // Debug: log token data if OMB_DEBUG is set
  if (process.env.OMB_DEBUG) {
...
async function main(): Promise<void> {
  try {
    ...
    // Debug: log data if OMB_DEBUG is set
    if (process.env.OMB_DEBUG) {
      console.error(
    ...
    if (isInstallError) {
      ...
      console.log("[OMB] HUD error - check stderr");
