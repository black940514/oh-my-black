...
/**
 * Get the HUD state file path in the project's .omb/state directory
 */
...
export function readHudState(directory?: string): OmcHudState | null {
  // Check new local state first (.omb/state/hud-state.json)
...
export function readHudConfig(): HudConfig {
  ...
  // 2. Try reading from ~/.claude/.omb/hud-config.json (legacy)
