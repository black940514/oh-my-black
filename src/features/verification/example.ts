...
export async function exampleRalphVerification(): Promise<void> {
  ...
  if (validation.valid) {
    ...
    console.log('[RALPH VERIFIED] All checks passed. Run /oh-my-black:cancel to exit.');
  } else {
...
export async function exampleAutopilotValidation(): Promise<void> {
  ...
  if (checklist.summary?.verdict === 'approved') {
    ...
    console.log('[AUTOPILOT VERIFIED] All checks passed. Run /oh-my-black:cancel to exit.');
  }
}
