...
// Validation types for subtask verification
export type ValidationType = 'self-only' | 'validator' | 'architect';

// Re-export OhmyblackEvidenceType for convenience (includes all evidence types)
export type { OhmyblackEvidenceType as VerificationEvidenceType } from '../verification/types.js';

export interface SubtaskValidation {
  /** Type of validation required */
  validationType: ValidationType;

  /** Validator agent type (for validationType='validator') */
  validatorAgent?: string;  // validator-syntax, validator-logic, validator-security, validator-integration

  /** Maximum retry attempts before escalation */
  maxRetries: number;

  /** Required evidence types for completion */
  evidenceRequired: OhmyblackEvidenceType[];
}
...
export interface Subtask {
  ...
  /** Validation configuration for this subtask */
  validation?: SubtaskValidation;
}

export interface ValidationStrategy {
  /** Mapping of subtask IDs to their validator types */
  subtaskValidators: Record<string, string>;

  /** Whether integration validation is required across subtasks */
  integrationValidation: boolean;

  /** Whether architect-level review is required for final approval */
...
export interface DecompositionResult {
  ...
  /** Validation strategy for the decomposition */
