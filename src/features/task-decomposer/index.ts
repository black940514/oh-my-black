...
import type { OhmyblackEvidenceType } from '../verification/types.js';
...
// Also export OhmyblackEvidenceType for users who need the full type
export type { OhmyblackEvidenceType } from '../verification/types.js';
...
export function generateSubtasks(
  ...
  return components.map((component) => {
    const subtask: Subtask = {
      ...
      validation: generateDefaultValidation(component, analysis)
    };
...
/**
 * Generate default validation configuration for a subtask
 */
function generateDefaultValidation(
  component: Component,
  analysis: TaskAnalysis
): SubtaskValidation {
  // Determine validation type based on component complexity and role
  const validationType: ValidationType =
    component.effort >= 0.7 ? 'architect' :
    component.effort >= 0.4 ? 'validator' :
    'self-only';

  // Select evidence types based on component role
  const evidenceRequired: OhmyblackEvidenceType[] = ['build_success', 'lint_clean'];

  if (component.role === 'testing') {
    evidenceRequired.push('test_pass');
  }

  if (component.role === 'frontend' || component.role === 'ui') {
    evidenceRequired.push('syntax_clean');
  }
  ...
  // Add integration pass for components with dependencies
  if (component.dependencies.length > 0) {
    evidenceRequired.push('integration_pass');
  }

  // Select validator agent based on component role
  let validatorAgent: string | undefined;
  if (validationType === 'validator') {
    validatorAgent = selectValidatorAgent(component);
  }
...
/**
 * Select appropriate validator agent based on component role
 */
function selectValidatorAgent(component: Component): string {
  switch (component.role) {
    case 'frontend':
    case 'ui':
      return 'validator-syntax';
    case 'backend':
    case 'api':
...
function selectAgentType(component: Component): string {
  const roleToAgent: Record<ComponentRole, string> = {
    frontend: 'oh-my-black:designer',
    backend: 'oh-my-black:executor',
    database: 'oh-my-black:executor',
    api: 'oh-my-black:executor',
    ui: 'oh-my-black:designer',
    shared: 'oh-my-black:executor',
    testing: 'oh-my-black:qa-tester',
    docs: 'oh-my-black:writer',
    config: 'oh-my-black:executor',
    module: 'oh-my-black:executor'
  };

  return roleToAgent[component.role] || 'oh-my-black:executor';
}
