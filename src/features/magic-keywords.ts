...
|------|---------|---------|
| Write/Edit | \`.omb/**/*.md\` ONLY | Everything else |
| Read | All files | - |
| Bash | Research commands only | Implementation commands |
| Task | explore, researcher | - |

**IF YOU TRY TO WRITE/EDIT OUTSIDE \`.omb/\`:**
- System will BLOCK your action
- You will receive an error
- DO NOT retry - you are not supposed to implement

**YOUR ONLY WRITABLE PATHS:**
- \`.omb/plans/*.md\` - Final work plans
- \`.omb/drafts/*.md\` - Working drafts during interview
 ...
 * Handles both ASCII and Unicode (e.g., Korean, Japanese) triggers
 */
function removeTriggerWords(prompt: string, triggers: string[]): string {
  let result = prompt;
  for (const trigger of triggers) {
    // For non-ASCII characters (Korean, Japanese, etc.), use lookahead/lookbehind without \b
    const isUnicode = /[^\x00-\x7F]/.test(trigger);
    const regex = isUnicode
      ? new RegExp(trigger, 'gi')
      : new RegExp(`\\b${trigger}\\b`, 'gi');
...
/**
 * Korean intent detection: autopilot mode
 * Detects action-oriented Korean verbs and activates autopilot
 */
const autopilotKoreanEnhancement: MagicKeyword = {
  triggers: ['해줘', '만들어', '구현해', '추가해', '수정해', '고쳐줘', '작성해'],
  description: 'Activates autopilot mode for Korean action requests',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['해줘', '만들어', '구현해', '추가해', '수정해', '고쳐줘', '작성해']);
    return `<autopilot-mode>
AUTOPILOT MODE ACTIVATED.
Execute the user's request autonomously with full B-V verification.
</autopilot-mode>
...
/**
 * Korean intent detection: ralplan mode
 * Detects planning-oriented Korean words and activates ralplan
 */
const ralplanKoreanEnhancement: MagicKeyword = {
  triggers: ['계획', '어떻게', '방법', '전략', '접근'],
  description: 'Activates ralplan mode for Korean planning requests',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['계획', '어떻게', '방법', '전략', '접근']);
    return `<ralplan-mode>
RALPLAN MODE ACTIVATED.
Iterative planning with Planner + Architect + Critic consensus.
</ralplan-mode>
...
/**
 * Korean intent detection: ultrapilot mode
 * Detects speed/urgency-oriented Korean words and activates ultrapilot
 */
const ultrapilotKoreanEnhancement: MagicKeyword = {
  triggers: ['빨리', '급해', '병렬', '동시에', '빠르게'],
  description: 'Activates ultrapilot mode for Korean speed requests',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['빨리', '급해', '병렬', '동시에', '빠르게']);
    return `<ultrapilot-mode>
ULTRAPILOT MODE ACTIVATED.
Parallel autopilot execution with 3-5 concurrent workers for maximum speed.
</ultrapilot-mode>
...
/**
 * Korean intent detection: ralph persistence mode
 * Detects persistence-oriented Korean words and activates ralph
 */
const ralphKoreanEnhancement: MagicKeyword = {
  triggers: ['끝까지', '완료', '멈추지', '포기하지마'],
  description: 'Activates ralph persistence mode for Korean completion requests',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['끝까지', '완료', '멈추지', '포기하지마']);
    return `<ralph-mode>
RALPH MODE ACTIVATED.
Persistence mode: continue until fully verified complete.
</ralph-mode>
...
/**
 * Explicit autopilot mode keyword
 */
const autopilotExplicitEnhancement: MagicKeyword = {
  triggers: ['autopilot'],
  description: 'Explicit autopilot mode activation',
  action: (prompt: string) => {
...
/**
 * Explicit ralph mode keyword
 */
const ralphExplicitEnhancement: MagicKeyword = {
  triggers: ['ralph'],
  description: 'Explicit ralph persistence mode activation',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['ralph']);
    return `<ralph-mode>
RALPH MODE ACTIVATED.
Persistence mode: continue until fully verified complete.
</ralph-mode>
...
/**
 * Explicit plan mode keyword
 */
const planExplicitEnhancement: MagicKeyword = {
  triggers: ['plan'],
  description: 'Explicit plan mode activation',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['plan']);
    return `<plan-mode>
PLAN MODE ACTIVATED.
Start planning interview with user to gather requirements.
</plan-mode>
...
/**
 * Explicit ralplan mode keyword
 */
const ralplanExplicitEnhancement: MagicKeyword = {
  triggers: ['ralplan'],
  description: 'Explicit ralplan mode activation',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['ralplan']);
    return `<ralplan-mode>
RALPLAN MODE ACTIVATED.
Iterative planning with Planner + Architect + Critic consensus.
</ralplan-mode>
...
/**
 * Explicit ecomode keyword
 */
const ecoExplicitEnhancement: MagicKeyword = {
  triggers: ['eco', 'ecomode'],
  description: 'Explicit ecomode activation',
  action: (prompt: string) => {
    const cleanPrompt = removeTriggerWords(prompt, ['eco', 'ecomode']);
    return `<ecomode>
ECOMODE ACTIVATED.
Token-efficient parallel execution with budget optimization.
</ecomode>
...
export const builtInMagicKeywords: MagicKeyword[] = [
  ...
  // Zero Learning Curve: Korean intent detection
  autopilotKoreanEnhancement,
  ralplanKoreanEnhancement,
  ultrapilotKoreanEnhancement,
  ralphKoreanEnhancement,
  // Explicit mode keywords
  autopilotExplicitEnhancement,
  ralphExplicitEnhancement,
  planExplicitEnhancement,
  ralplanExplicitEnhancement,
  ecoExplicitEnhancement
];
  ...
  return (prompt: string): string => {
    ...
    for (const keyword of keywords) {
      const hasKeyword = keyword.triggers.some(trigger => {
        // For non-ASCII characters (Korean, Japanese, etc.), don't use \b word boundaries
        const isUnicode = /[^\x00-\x7F]/.test(trigger);
        const regex = isUnicode
