...
export interface EnforcementResult {
  ...
  model: ModelType;
  /** Warning message (only if OMB_DEBUG=true) */
  warning?: string;
}
...
export function enforceModel(agentInput: AgentInput): EnforcementResult {
  ...
  // Extract agent type (strip oh-my-black: prefix if present)
  const agentType = agentInput.subagent_type.replace(/^oh-my-black:/, '');
  ...
  // Create warning message (only shown if OMB_DEBUG=true)
  let warning: string | undefined;
  if (process.env.OMB_DEBUG === 'true') {
    warning = `[OMB] Auto-injecting model: ${sdkModel} for ${agentType}`;
  }
 ...
 *
 * @param agentType - The agent type (with or without oh-my-black: prefix)
...
export function getModelForAgent(agentType: string): ModelType {
  const normalizedType = agentType.replace(/^oh-my-black:/, '');
  const agentDefs = getAgentDefinitions();
