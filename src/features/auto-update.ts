 ...
 * Auto-Update System
 *
 * Provides version checking and auto-update functionality for oh-my-black.
 *
 * Features:
...
export const REPO_OWNER = 'black940514';
export const REPO_NAME = 'oh-my-black';
...
/**
 * OMB configuration (stored in .omb-config.json)
 */
export interface SisyphusConfig {
  ...
  /** Preferred execution mode for parallel work (set by omb-setup Step 3.7) */
...
export function getInstalledVersion(): VersionMetadata | null {
  if (!existsSync(VERSION_FILE)) {
    ...
    try {
      ...
      const result = execSync('npm list -g oh-my-black --json', {
        encoding: 'utf-8',
        timeout: 5000,
        stdio: 'pipe'
      });
      const data = JSON.parse(result);
      if (data.dependencies?.['oh-my-black']?.version) {
        return {
          version: data.dependencies['oh-my-black'].version,
...
export async function fetchLatestRelease(): Promise<ReleaseInfo> {
  const response = await fetch(`${GITHUB_API_URL}/releases/latest`, {
    headers: {
      ...
      'User-Agent': 'oh-my-black-updater'
    }
  });
  ...
  try {
    ...
    if (!response.ok) {
      ...
      throw new Error(
        ...
        'Please update using: /plugin install oh-my-black'
      );
    }
    ...
    try {
      ...
      if (isWindows) {
        ...
        try {
          execSync('npm install -g oh-my-black@latest', {
            encoding: 'utf-8',
          ...
          throw new Error(
            ...
            '  npm install -g oh-my-black@latest\n' +
...
export function formatUpdateNotification(checkResult: UpdateCheckResult): string {
  if (!checkResult.updateAvailable) {
    return `oh-my-black is up to date (v${checkResult.currentVersion ?? 'unknown'})`;
  }

  const lines = [
    '╔═══════════════════════════════════════════════════════════╗',
    '║           oh-my-black Update Available!              ║',
    '╚═══════════════════════════════════════════════════════════╝',
    '',
    ...
    '  Or reinstall via: /plugin install oh-my-black',
    ''
  ];
