...
export function loadEnvConfig(): Partial<PluginConfig> {
  ...
  if (process.env.OMB_PARALLEL_EXECUTION !== undefined) {
    config.features = {
      ...config.features,
      parallelExecution: process.env.OMB_PARALLEL_EXECUTION === 'true'
    };
  }

  if (process.env.OMB_LSP_TOOLS !== undefined) {
    config.features = {
      ...config.features,
      lspTools: process.env.OMB_LSP_TOOLS === 'true'
    };
  }

  if (process.env.OMB_MAX_BACKGROUND_TASKS) {
    const maxTasks = parseInt(process.env.OMB_MAX_BACKGROUND_TASKS, 10);
    if (!isNaN(maxTasks)) {
      config.permissions = {
        ...config.permissions,
  ...
  if (process.env.OMB_ROUTING_ENABLED !== undefined) {
    config.routing = {
      ...config.routing,
      enabled: process.env.OMB_ROUTING_ENABLED === 'true'
    };
  }

  if (process.env.OMB_ROUTING_DEFAULT_TIER) {
    const tier = process.env.OMB_ROUTING_DEFAULT_TIER.toUpperCase();
  ...
  if (process.env.OMB_ESCALATION_ENABLED !== undefined) {
    config.routing = {
      ...config.routing,
      escalationEnabled: process.env.OMB_ESCALATION_ENABLED === 'true'
    };
  }
...
export function generateConfigSchema(): object {
  return {
    ...
    title: 'Oh-My-Black Configuration',
    type: 'object',
    properties: {
      agents: {
        type: 'object',
