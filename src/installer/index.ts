 ...
 * Installer Module
 *
 * Handles installation of OMB agents, commands, and configuration
...
/**
 * Detect whether a hook command belongs to oh-my-black.
 *
 * Uses substring matching rather than word-boundary regex.
 * Rationale: Real OMB hooks use compound names where "omc" is embedded
 * (e.g., `omc-pre-tool-use.mjs`, `oh-my-black-hook.mjs`). A word-boundary
 * regex like /\bomc\b/ would fail to match "oh-my-black" since "omc" appears
 ...
 * @returns true if the command contains 'omc' or 'oh-my-black'
 */
export function isOmcHook(command: string): boolean {
  ...
  const fullNamePattern = /oh-my-black/;
...
/**
 * Install OMB agents, commands, skills, and hooks
 */
  ...
  try {
    ...
    if (!runningAsPlugin) {
      ...
      if (!existsSync(homeMdPath)) {
        ...
        // Merge OMB content with existing content
    ...
    if (!projectScoped) try {
      ...
      const hudScriptLines = [
        ...
        '  ',
        '  // 1. Development paths (only when OMB_DEV=1)',
        '  if (process.env.OMB_DEV === "1") {',
        '    const devPaths = [',
        '      join(home, "Workspace/oh-my-black/dist/hud/index.js"),',
        '      join(home, "workspace/oh-my-black/dist/hud/index.js"),',
        '      join(home, "projects/oh-my-black/dist/hud/index.js"),',
        '    ];',
        '    ',
        ...
        '  try {',
        '    await import("oh-my-black/dist/hud/index.js");',
        '    return;',
        '  } catch { /* continue */ }',
        '  ',
        ...
        '      console.log(`[OMB HUD] Plugin installed but not built. Run: cd "${pluginCacheDir}" && npm install && npm run build`);',
        '    } else {',
        '      console.log(`[OMB HUD] Plugin dist/ exists but HUD not found. Run: cd "${pluginCacheDir}" && npm run build`);',
        '    }',
        '  } else if (existsSync(pluginCacheBase)) {',
        '    // Plugin cache directory exists but no versions',
        '    console.log(`[OMB HUD] Plugin cache found but no versions installed. Run: /oh-my-black:omb-setup`);',
        '  } else {',
        '    // No plugin installation found at all',
        '    console.log("[OMB HUD] Plugin not installed. Run: /oh-my-black:omb-setup");',
        '  }',
        '}',
        '',
        'main();',
        ...
        for (const [eventType, eventHooks] of Object.entries(newHooks)) {
          if (!existingHooks[eventType]) {
            ...
            if (hasNonOmcHook && !options.forceHooks) {
              ...
              log(`  [OMB] Warning: ${eventType} hook owned by another plugin. Skipping. Use --force-hooks to override.`);
...
/**
 * Check if OMB is already installed
 */
