...
describe('mergeClaudeMd', () => {
  const omcContent = '# OMB Configuration\n\nThis is the OMB content.';
  ...
  describe('Update existing content with markers', () => {
    it('replaces only content between markers', () => {
      ...
      expect(result).not.toContain('Old OMB Content');
  ...
  describe('No markers in existing content', () => {
    it('wraps omcContent in markers and preserves existing content after user customizations header', () => {
      ...
      // Check order: OMB section first, then user customizations header, then existing content
  ...
  describe('Real-world scenarios', () => {
    it('handles typical fresh install scenario', () => {
      const result = mergeClaudeMd(null, omcContent);
      expect(result).toMatch(/^<!-- OMB:START -->\n.*\n<!-- OMB:END -->\n$/s);
    });
...
User added custom stuff here`;
      ...
      // New OMB content should be at the top with markers
  ...
  describe('idempotency guard', () => {
    it('strips markers from omcContent that already has markers', () => {
      ...
      const omcWithMarkers = `<!-- OMB:START -->
# oh-my-black
...
<!-- OMB:END -->`;
      ...
      const startCount = (result.match(/<!-- OMB:START -->/g) || []).length;
      const endCount = (result.match(/<!-- OMB:END -->/g) || []).length;
    ...
    it('handles omcContent with markers when merging into existing content', () => {
      const existingContent = `<!-- OMB:START -->
Old OMB content
<!-- OMB:END -->
...
<!-- OMB:END -->`;
      ...
      expect(startCount).toBe(1);
      expect(endCount).toBe(1);
      expect(result).toContain('New OMB content v2');
      expect(result).not.toContain('Old OMB content');
