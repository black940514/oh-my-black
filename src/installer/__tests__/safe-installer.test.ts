...
describe('isOmcHook', () => {
  ...
  it('returns true for commands containing "oh-my-black"', () => {
    expect(isOmcHook('node ~/.claude/hooks/oh-my-black-hook.mjs')).toBe(true);
    expect(isOmcHook('bash $HOME/.claude/hooks/oh-my-black.sh')).toBe(true);
  });

  it('returns false for commands not containing omc or oh-my-black', () => {
...
describe('isOmcHook detection', () => {
  ...
  it('rejects non-OMB hooks correctly', () => {
...
describe('Safe Installer - Hook Conflict Detection', () => {
  ...
  it('detects conflict when PreToolUse is owned by another plugin', () => {
    // Create settings.json with non-OMB hook
  ...
  it('does not detect conflict when hook is OMB-owned', () => {
    const existingSettings = {
      hooks: {
        PreToolUse: [
          {
