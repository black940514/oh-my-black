 ...
 *
 * All modes store state in `.omb/state/` subdirectory for consistency.
 */
 ...
 * All paths are relative to .omb/state/ directory.
 */
 ...
 * @deprecated Global state is no longer supported. All modes use local-only state in .omb/state/
...
/**
 * Check if any OMB mode is currently active
 *
 * @param cwd - Working directory
    ...
    for (const exclusiveMode of EXCLUSIVE_MODES) {
      if (exclusiveMode !== mode && isModeActive(exclusiveMode, cwd)) {
        ...
        return {
          ...
          message: `Cannot start ${MODE_CONFIGS[mode].name} while ${config.name} is active. Cancel ${config.name} first with /oh-my-black:cancel.`
        };
      }
    }
  }
 ...
 *
 * Deletes:
 * - Local state file (.omb/state/{mode}-state.json)
