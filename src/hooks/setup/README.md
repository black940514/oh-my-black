...
Handles OMB initialization and maintenance tasks.

## Triggers

### `init`
Initializes OMB directory structure and environment on first run or explicit setup.

**What it does:**
- Creates required directories: `.omb/state/`, `.omb/logs/`, `.omb/notepads/`, `.omb/state/checkpoints/`, `.omb/plans/`
- Validates existing config files (`.omb-config.json`)
- Sets environment variables (`OMB_INITIALIZED=true`) if `CLAUDE_ENV_FILE` is available
...
```json
{
  ...
  "hookSpecificOutput": {
    "hookEventName": "Setup",
    "additionalContext": "OMB initialized:\n- 5 directories created\n- 1 configs validated\n- Environment variables set: OMB_INITIALIZED"
  }
}
```

### `maintenance`
Performs periodic maintenance tasks to keep OMB state clean.
...
// => ['/path/to/project/.omb-config.json']
```

### Environment Variables

#### `setEnvironmentVariables(): string[]`
Sets environment variables for OMB initialization.
