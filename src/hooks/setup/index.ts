...
/**
 * Set environment variables for OMB initialization
 */
export function setEnvironmentVariables(): string[] {
  ...
  if (process.env.CLAUDE_ENV_FILE) {
    try {
      const envContent = `export OMB_INITIALIZED=true\n`;
      appendFileSync(process.env.CLAUDE_ENV_FILE, envContent);
      envVars.push('OMB_INITIALIZED');
    } catch {
...
/**
 * Prune old state files from .omb/state directory
 */
...
export function vacuumSwarmDb(directory: string): boolean {
  const swarmDbPath = join(directory, '.omb/state/swarm.db');
