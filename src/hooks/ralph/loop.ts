 ...
 * Ralph Hook
 *
 * Self-referential work loop that continues until cancelled via /oh-my-black:cancel.
...
export function createRalphLoopHook(directory: string): RalphLoopHook {
  const startLoop = (
    ...
    if (isUltraQAActive(directory)) {
      console.error(
        "Cannot start Ralph Loop while UltraQA is active. Cancel UltraQA first with /oh-my-black:cancel.",
      );
      return false;
    }
