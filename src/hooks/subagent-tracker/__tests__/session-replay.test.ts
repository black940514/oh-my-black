...
describe('session-replay', () => {
  ...
  describe('getReplayFilePath', () => {
    it('should return correct path for session', () => {
      ...
      expect(path).toContain(join('.omb', 'state', 'agent-replay-test-session.jsonl'));
    });
  ...
  describe('event helpers', () => {
    it('recordAgentStart should record start event', () => {
      recordAgentStart(testDir, 'sess3', 'agent-123', 'oh-my-black:executor', 'Fix the bug', 'ultrawork', 'sonnet');
    ...
    it('recordAgentStop should record stop event', () => {
      recordAgentStop(testDir, 'sess4', 'agent-456', 'oh-my-black:architect', true, 5000);
