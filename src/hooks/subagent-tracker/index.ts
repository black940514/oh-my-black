...
function acquireLock(directory: string): boolean {
  const lockPath = join(directory, ".omb", "state", "subagent-tracker.lock");
  const lockDir = join(directory, ".omb", "state");
...
export function getAgentDashboard(directory: string): string {
  ...
  for (const agent of running) {
    ...
    const shortType = agent.agent_type.replace("oh-my-black:", "");
...
export function getAgentObservatory(directory: string): {
  ...
  for (const intervention of interventions.slice(0, 3)) {
    const shortType = intervention.agent_type.replace("oh-my-black:", "");
    ...
    if (agents.length > 1) {
      ...
      for (let i = 1; i < agents.length; i++) {
        interventions.push({
          ...
          reason: `File conflict on ${file} with ${agents[0].type.replace("oh-my-black:", "")}`,
          suggested_action: "warn",
          auto_execute: false,
        });
      }
...
export function detectFileConflicts(directory: string): Array<{
  ...
  for (const agent of running) {
    for (const file of agent.file_ownership || []) {
      ...
      fileToAgents
        .get(file)!
        .push(agent.agent_type.replace("oh-my-black:", ""));
    }
  }
