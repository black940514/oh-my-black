 ...
 */
export async function integrateResults(cwd: string): Promise<IntegrationResult> {
  const state = readUltrapilotState(cwd);
  if (!state) {
    return {
      success: false,
 ...
 */
function detectFileConflicts(state: UltrapilotState): string[] {
 ...
 */
export function isFileOwnedByWorker(
  cwd: string,
  workerId: string,
  filePath: string
): boolean {
 ...
 */
export function isSharedFile(cwd: string, filePath: string): boolean {
  const state = readUltrapilotState(cwd);
  if (!state) return false;
