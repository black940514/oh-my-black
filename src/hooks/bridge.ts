...
- When FULLY complete (after Architect verification), run \`/oh-my-black:cancel\` to cleanly exit and clean up state files. If cancel fails, retry with \`/oh-my-black:cancel --force\`.
      ...
      if (runningCount >= maxBgTasks) {
        return {
          ...
          reason:
            ...
            `Limit is configurable via permissions.maxBackgroundTasks in config or OMB_MAX_BACKGROUND_TASKS env var.`,
        };
      }
    }
  }
...
function processAutopilot(input: HookInput): HookOutput {
  ...
  const context = {
    ...
    specPath: state.expansion.spec_path || ".omb/autopilot/spec.md",
    planPath: state.planning.plan_path || ".omb/plans/autopilot-impl.md",
  };
...
/**
 * Cached parsed OMB_SKIP_HOOKS for performance (env vars don't change during process lifetime)
 */
let _cachedSkipHooks: string[] | null = null;
function getSkipHooks(): string[] {
  if (_cachedSkipHooks === null) {
    _cachedSkipHooks =
      process.env.OMB_SKIP_HOOKS?.split(",")
        .map((s) => s.trim())
        .filter(Boolean) ?? [];
  }
...
export async function processHook(
  ...
  if (process.env.DISABLE_OMB === "1" || process.env.DISABLE_OMB === "true") {
    return { continue: true };
  }
