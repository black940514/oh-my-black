/**
 * Team Integration for Ultrawork
 *
 * Optional team composition for ultrawork mode when --auto-team flag is used.
 */

import { selectTeamTemplate } from '../../features/smart-defaults/team-selector.js';
import type { UltraworkConfig, UltraworkTeamConfig } from './types.js';

export type { UltraworkConfig, UltraworkTeamConfig } from './types.js';

/**
 * Check if auto-team composition should be used
 */
export function shouldUseAutoTeam(config: UltraworkConfig): boolean {
  return config.autoTeam === true;
}

/**
 * Compose team for ultrawork based on task description and complexity
 */
export function composeTeamForUltrawork(
 ...
 */
function getTeamByTemplate(template: string): UltraworkTeamConfig {
  switch (template) {
    case 'minimal':
      return {
        agents: ['executor-low'],
