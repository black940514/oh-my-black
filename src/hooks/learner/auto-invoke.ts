...
/**
 * Load auto-invocation config from ~/.claude/.omb-config.json
 */
export function loadInvocationConfig(): InvocationConfig {
  const configPath = path.join(os.homedir(), '.claude', '.omb-config.json');
...
export function saveInvocationHistory(state: AutoInvokeState): void {
  const historyDir = path.join(os.homedir(), '.omb', 'analytics', 'invocations');
