        ...
        for (const group of eventHookGroups) {
          ...
          for (const hook of group.hooks) {
            if (hook.type === 'command' && hook.command) {
              ...
              const isOmc = lowerCmd.includes('omc') || lowerCmd.includes('oh-my-black');
...
/**
 * Check CLAUDE.md for OMB markers and user content
 */
export function checkClaudeMdStatus(): ConflictReport['claudeMdStatus'] {
  ...
  try {
    ...
    const hasStartMarker = content.includes('<!-- OMB:START -->');
    const hasEndMarker = content.includes('<!-- OMB:END -->');
    ...
    if (hasMarkers) {
      ...
      const startIdx = content.indexOf('<!-- OMB:START -->');
      const endIdx = content.indexOf('<!-- OMB:END -->');

      const beforeMarker = content.substring(0, startIdx).trim();
      const afterMarker = content.substring(endIdx + '<!-- OMB:END -->'.length).trim();
...
/**
 * Check environment flags that affect OMB behavior
 */
export function checkEnvFlags(): ConflictReport['envFlags'] {
  const disableOmc = process.env.DISABLE_OMB === 'true' || process.env.DISABLE_OMB === '1';
  const skipHooks: string[] = [];

  if (process.env.OMB_SKIP_HOOKS) {
    skipHooks.push(...process.env.OMB_SKIP_HOOKS.split(',').map(h => h.trim()));
  }
...
export function checkConfigIssues(): ConflictReport['configIssues'] {
  ...
  const configPath = join(homedir(), '.claude', '.omb-config.json');
  ...
  try {
    ...
    const knownFields = new Set([
      ...
      'magicKeywords',
      'routing',
      // SisyphusConfig fields (from auto-update.ts / omb-setup)
      'silentAutoUpdate',
      'configuredAt',
      'configVersion',
      'taskTool',
...
export function runConflictCheck(): ConflictReport {
  ...
  const hasConflicts =
    hookConflicts.some(h => !h.isOmc) || // Non-OMB hooks present
    envFlags.disableOmc || // OMB is disabled
    envFlags.skipHooks.length > 0 || // Hooks are being skipped
    configIssues.unknownFields.length > 0; // Unknown config fields
    // Note: Missing OMB markers is informational (normal for fresh install), not a conflict
...
export function formatReport(report: ConflictReport, json: boolean): string {
  ...
  if (report.hookConflicts.length > 0) {
    ...
    for (const hook of report.hookConflicts) {
      const status = hook.isOmc ? colors.green('✓ OMB') : colors.yellow('⚠ Other');
  ...
  if (report.claudeMdStatus) {
    ...
    if (report.claudeMdStatus.hasMarkers) {
      lines.push(`  ${colors.green('✓')} OMB markers present`);
      if (report.claudeMdStatus.hasUserContent) {
        lines.push(`  ${colors.green('✓')} User content preserved outside markers`);
      }
    } else {
      lines.push(`  ${colors.yellow('⚠')} No OMB markers found`);
      lines.push(`    ${colors.gray('Run /oh-my-black:omb-setup to add markers')}`);
      if (report.claudeMdStatus.hasUserContent) {
  ...
  if (report.envFlags.disableOmc) {
    lines.push(`  ${colors.red('✗')} DISABLE_OMB is set - OMB is disabled`);
  } else {
    lines.push(`  ${colors.green('✓')} DISABLE_OMB not set`);
  }

  if (report.envFlags.skipHooks.length > 0) {
    lines.push(`  ${colors.yellow('⚠')} OMB_SKIP_HOOKS: ${report.envFlags.skipHooks.join(', ')}`);
  } else {
  ...
  if (report.configIssues.unknownFields.length > 0) {
    ...
    lines.push('');
    lines.push(`  ${colors.yellow('⚠')} Unknown fields in .omb-config.json:`);
  ...
  if (report.hasConflicts) {
    ...
    lines.push(`${colors.gray('Review the issues above and run /oh-my-black:omb-setup if needed')}`);
  } else {
    lines.push(`${colors.green('✓')} No conflicts detected`);
    lines.push(`${colors.gray('OMB is properly configured')}`);
  }
  lines.push('');
