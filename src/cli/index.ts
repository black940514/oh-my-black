...
Examples:
  ...
  .action(async (options) => {
    ...
    console.log(chalk.gray('Configuration is now managed automatically. Use /oh-my-black:omb-setup instead.\n'));
...
Examples:
  ...
  .action(async () => {
    ...
    console.log(chalk.gray('\n━'.repeat(50)));
    console.log(chalk.gray('\nTo check for updates, run: oh-my-black update --check'));
  });
...
const doctorCmd = program
  .command('doctor')
  .description('Diagnostic tools for troubleshooting OMB installation')
  .addHelpText('after', `
Examples:
...
program
  ...
  .action(async () => {
    ...
    if (result.success) {
      ...
      console.log(chalk.gray('  Run "oh-my-black info" to see available agents.'));
      ...
      console.warn(chalk.gray('  Run "oh-my-black install" manually to complete setup.'));
    }
  });
