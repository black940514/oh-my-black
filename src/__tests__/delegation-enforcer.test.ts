...
describe('delegation-enforcer', () => {
  ...
  beforeEach(() => {
    originalDebugEnv = process.env.OMB_DEBUG;
  });
  ...
  afterEach(() => {
    if (originalDebugEnv === undefined) {
      ...
      process.env.OMB_DEBUG = originalDebugEnv;
    }
  });
  ...
  describe('enforceModel', () => {
    ...
    it('logs warning only when OMB_DEBUG=true', () => {
      const input: AgentInput = {
        description: 'Test task',
        prompt: 'Do something',
    ...
    it('does not log warning when OMB_DEBUG is false', () => {
      const input: AgentInput = {
        description: 'Test task',
        prompt: 'Do something',
        subagent_type: 'executor'
  ...
  describe('processPreToolUse', () => {
    ...
    it('logs warning only when OMB_DEBUG=true and model injected', () => {
      const toolInput: AgentInput = {
        description: 'Test',
        prompt: 'Test',
        subagent_type: 'executor'
  ...
  describe('getModelForAgent', () => {
    it('returns correct model for agent with prefix', () => {
      expect(getModelForAgent('oh-my-black:executor')).toBe('sonnet');
      expect(getModelForAgent('oh-my-black:executor-low')).toBe('haiku');
      expect(getModelForAgent('oh-my-black:architect')).toBe('opus');
    });
