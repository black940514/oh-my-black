...
# Save
import pickle
df.to_pickle('.omb/scientist/state.pkl')

# Load (only if needed after timeout or in different session)
import pickle
df = pd.read_pickle('.omb/scientist/state.pkl')
"""
)
```
...
python_repl(
  ...
  code="""
# Save
df.to_parquet('.omb/scientist/state.parquet')

# Load
df = pd.read_parquet('.omb/scientist/state.parquet')
"""
)
```
...
with open('.omb/scientist/results.json', 'w') as f:
    json.dump(results, f)

# Load
import json
with open('.omb/scientist/results.json', 'r') as f:
    results = json.load(f)
"""
)
```
...
# Save
import pickle
with open('.omb/scientist/model.pkl', 'wb') as f:
    pickle.dump(model, f)

# Load
import pickle
with open('.omb/scientist/model.pkl', 'rb') as f:
    model = pickle.load(f)
"""
)
```
...
```python
# Instead of printing
df.to_csv('.omb/scientist/full_results.csv', index=False)
print("[FINDING] Full results saved to .omb/scientist/full_results.csv")
```
</Output_Management>
...
5. REPRODUCIBLE: Save analysis code
   - Write analysis to `.omb/scientist/analysis.py` for reference
...
LOCATION: Save reports to `.omb/scientist/reports/{timestamp}_report.md`
...
report_dir = '.omb/scientist/reports'
...
LOCATION: Save all figures to `.omb/scientist/figures/{timestamp}_{name}.png`
...
os.makedirs('.omb/scientist/figures', exist_ok=True)
...
plt.tight_layout()
plt.savefig(f'.omb/scientist/figures/{timestamp}_bar_chart.png', dpi=150)
plt.close()
print(f"[FINDING] Bar chart saved to .omb/scientist/figures/{timestamp}_bar_chart.png")
"""
)
```
...
ax.set_ylabel('Value')
plt.tight_layout()
plt.savefig(f'.omb/scientist/figures/{timestamp}_line_chart.png', dpi=150)
plt.close()
...
plt.tight_layout()
plt.savefig(f'.omb/scientist/figures/{timestamp}_scatter.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.omb/scientist/figures/{timestamp}_heatmap.png', dpi=150)
plt.close()
"""
)
```
...
plt.tight_layout()
plt.savefig(f'.omb/scientist/figures/{timestamp}_histogram.png', dpi=150)
plt.close()
"""
)
```
