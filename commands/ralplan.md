...
Ralplan maintains persistent state in `.omb/ralplan-state.json` to track progress and enable recovery across interruptions:
...
```json
{
  ...
  "iteration": 1,
  "max_iterations": 5,
  "plan_path": ".omb/plans/[feature].md",
...
1. Create `.omb/plans/` directory if it doesn't exist
...
- Planner generates plan directly to `.omb/plans/[feature-name].md`
...
- Signal completion with `PLAN_READY: .omb/plans/[filename].md`
...
- Prepare plan for execution with `/oh-my-black:ralph` or manual orchestration
...
- Use `/oh-my-black:cancel` (automatically detects active mode)
- Or manually delete `.omb/ralplan-state.json`
