...
Run `/oh-my-black:omb-setup` to set ecomode as your default parallel execution mode.

When set as default, saying "fast" or "parallel" will activate ecomode instead of ultrawork.

## Disabling Ecomode

To completely disable ecomode (prevents all ecomode keyword detection and activation), add to `~/.claude/.omb-config.json`:
...
- `/oh-my-black:cancel` - Cancel active mode
