---
description: One-time setup for oh-my-black (the ONLY command you need to learn)
---
...
curl -fsSL "https://raw.githubusercontent.com/black940514/oh-my-black/main/docs/CLAUDE.md" -o .claude/CLAUDE.md && \
...
PLUGIN_DIR="$HOME/.claude/plugins/cache/omc/oh-my-black"
if [ -d "$PLUGIN_DIR" ]; then
  ...
  echo "Plugin not installed - install with: claude /install-plugin oh-my-black"
fi
```
...
The OMB CLI provides standalone token analytics commands (`omc stats`, `omc agents`, `omc backfill`, `omc tui`).
...
if command -v bun &> /dev/null; then
  echo "Installing OMB CLI via bun..."
  # Clean up npm version if it exists to avoid duplicates
  if command -v npm &> /dev/null && npm list -g oh-my-black &>/dev/null; then
    echo "Removing existing npm installation to avoid duplicates..."
    npm uninstall -g oh-my-black 2>/dev/null
  fi
  bun install -g oh-my-black
elif command -v npm &> /dev/null; then
  echo "Installing OMB CLI via npm..."
...
if command -v omc &> /dev/null; then
  echo "✓ OMB CLI installed successfully!"
...
Skip this step. User can install later with `bun install -g oh-my-black` or `npm install -g oh-my-black`.
...
```bash
gh api -X PUT /user/starred/black940514/oh-my-black 2>/dev/null && echo "Thanks for starring! ⭐" || echo "Could not star - you can star manually at https://github.com/black940514/oh-my-black"
```
