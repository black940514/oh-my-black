...
**Path Exception**: Only write to `.omb/`, `.claude/`, `CLAUDE.md`, `AGENTS.md`
...
1. **First**, spawn Architect to verify your work (ALWAYS pass model explicitly!):
   ```
   Task(subagent_type="oh-my-black:architect", model="opus", prompt="Verify this implementation is complete: [describe what you did]")
   ```

2. **Wait for Architect's assessment**

3. **If Architect approves**: Run `/oh-my-black:cancel` to cleanly exit ralph mode
...
- When FULLY complete AND Architect verified: Run `/oh-my-black:cancel` to exit and clean up state
