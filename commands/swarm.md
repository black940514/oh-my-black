...
```
/oh-my-black:swarm N:agent-type "task description"
```
...
/oh-my-black:swarm 3:build-fixer "fix build errors in src/"
/oh-my-black:swarm 4:designer "implement responsive layouts for all components"
/oh-my-black:swarm 2:architect "analyze and document all API endpoints"
```
...
- User cancels via `/oh-my-black:cancel`

## Storage

### SQLite Database (`.omb/state/swarm.db`)

The swarm uses a single SQLite database stored at `.omb/state/swarm.db`. This provides:
...
- **Database:** SQLite (stored at `.omb/state/swarm.db`)
...
### Fix All Type Errors
```
/oh-my-black:swarm 5:executor "fix all TypeScript type errors"
```
Spawns 5 executors, each claiming and fixing individual files.

### Implement UI Components
```
/oh-my-black:swarm 3:designer "implement Material-UI styling for all components"
```
Spawns 3 designers, each styling different component files.

### Security Audit
```
/oh-my-black:swarm 4:security-reviewer "review all API endpoints for vulnerabilities"
```
Spawns 4 security reviewers, each auditing different endpoints.

### Documentation Sprint
```
/oh-my-black:swarm 2:writer "add JSDoc comments to all exported functions"
```
