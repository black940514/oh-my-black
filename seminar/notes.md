...
**The Reveal**
- "That's exactly what oh-my-black does. It transforms Claude Code from a single generalist assistant into a coordinated team of 32 specialists."
- "Today, I'm going to show you how this changes everything about AI-assisted development."

### Transition
"Let me start by showing you exactly what OMB is and how it thinks differently about AI assistance."
...
## Section 2: What is OMB? (8 min, Slides 4-8)

### Opening Line
"The fundamental insight behind OMB is simple: your AI should be a conductor, not a performer."
...
- "That's the shift OMB makes. Claude becomes the conductor, and 32 specialized agents are the orchestra."
...
- "AFTER: 'Build authentication for the API.' OMB automatically: explores codebase, analyzes requirements, generates plan, implements in parallel, writes tests, verifies. You're the product owner stating the goal."
...
### Opening Line
"Execution modes are where OMB's power becomes concrete. Each mode is optimized for a specific type of work. Let's explore all five."
...
### Transition
"These modes are powered by OMB's agent system. Let's look at how those 32 specialists are organized."

### Audience Engagement
- "Quick question: Which mode sounds most useful for YOUR daily work?" (Take 2-3 responses)
- "The beauty is you don't have to memorize this. Say 'fast parallel fixes' and OMB activates ultrawork. Say 'efficient batch fixes' and it activates ecomode."
...
- "OMB organizes agents into 13 specializations:"
...
- "How does OMB decide which agent to use? Three factors:"
...
### Opening Line
"Enough talking about it. Let's see OMB in action across five different scenarios."

### Demo 1: Autopilot - Full Feature Build (5 min, Slide 29)

**Setup**
- Have terminal ready with OMB installed
...
- Could be the OMB codebase itself with intentional errors
...
### Opening Line
"Powerful technology is useless if it's hard to use. OMB is designed for zero learning curve."
...
```
[OMB] Mode: ultrawork | Agents: 3 active | executor-1: fixing auth.ts | executor-2: fixing api.ts | architect: reviewing
```
- "At a glance you know:"
- "  Which mode is active"
- "  How many agents are working"
- "  What each agent is doing"
- "Installation: `/oh-my-black:hud setup` - adds to your shell prompt"

**Notepad Wisdom** (Slide 36)
- "OMB learns from every session via the notepad system:"
...
- "These persist across sessions. Your OMB gets smarter over time."

**Cost Analytics** (Slide 37)
- "OMB tracks token usage per session:"
- "See exactly how much each mode costs"
- "Compare ultrawork vs ecomode for your workload"
- "Audit logs at `.omb/logs/delegation-audit.jsonl`"
...
claude-code "/oh-my-black:omb-setup"
...
### Transition
"That's OMB. Let's recap and open for questions."
...
### Opening Line
"Let's recap what makes OMB transformative."
...
- "OMB transforms Claude Code from a single assistant into a coordinated team."
...
- "If you like it, share it with your team. OMB shines with real-world complexity."
...
**Answer:**
"OMB itself is completely free - it's MIT licensed open-source. What you pay for is the Claude API usage.
...
The key cost benefit: OMB's smart model routing saves you 30-50% on token costs compared to manually using Claude. For example, simple searches use Haiku (super cheap), complex debugging uses Opus (expensive but necessary). Without OMB, everything might default to Opus.
...
**Answer:**
"Currently OMB is designed specifically for Claude Code and leverages Claude's three model tiers - Haiku, Sonnet, and Opus.
...
**Answer:**
"Great question. Without OMB, Claude Code gives you one very smart generalist assistant. You tell it every step: 'search for this, analyze that, now implement this, now test that.'

With OMB, you get 32 specialized agents orchestrated automatically. You state the goal - 'build authentication' - and OMB:
...
Real-world impact: Tasks that took 30 minutes of back-and-forth with Claude Code now take 5 minutes of autonomous execution with OMB."

### 4. "What about security? Is my code safe?"

**Answer:**
"OMB runs entirely locally via Claude Code. Your code never leaves your machine except through the normal Claude API calls that you'd be making anyway.

Additionally, OMB includes a security-reviewer agent that can audit code for common vulnerabilities. You can invoke it explicitly: '/pipeline security' runs a security audit pipeline.

The notepad wisdom system stores data locally in `.omb/notepads/`. Nothing is sent to external servers.
...
Then invoke: `Task(subagent_type="oh-my-black:pytest-specialist")`

You can also customize execution modes, delegation categories, and model routing rules via the config file at `~/.claude/.omb-config.json`.
...
**Answer:**
"Yes. OMB works with any language that Claude Code supports - which is basically all mainstream languages.
...
**Answer:**
"Honestly? You don't need to think about it. Just describe what you want in natural language and OMB auto-detects the right mode.
...
But more seriously - OMB has built-in verification at multiple levels:
...
In practice, failures happen - maybe a test fails, maybe there's a linting error. OMB catches these and fixes them before claiming completion.
...
**Answer:**
"OMB is designed for interactive development, not CI/CD automation.
...
That said, some teams use OMB-generated tests in their CI pipeline. The tests themselves are standard - jest, pytest, etc. - they just happened to be generated via OMB.
...
OMB: Literally zero learning time. If you can describe what you want in English, you can use OMB.
...
"Let me show you the mental model shift. BEFORE OMB: [read the before section slowly]. You're the micromanager. AFTER OMB: [read the after section with rising energy]. You're the product owner. This isn't a 10% improvement - this is a complete paradigm shift."
...
3. Feel confident they could install and use OMB today
4. Be excited about the paradigm shift from interactive to autonomous

**Your Success Metric:**
"How many people install OMB in the next week?"
