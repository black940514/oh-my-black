...
Complete reference for oh-my-black. For quick start, see the main [README.md](../README.md).
...
- **New projects**: Run `/oh-my-black:omb-setup --local` in each project that needs omc

> **NOTE**: After updating the plugin (via `npm update`, `git pull`, or Claude Code's plugin update), you MUST re-run `/oh-my-black:omb-setup` to apply the latest CLAUDE.md changes.
...
Always use `oh-my-black:` prefix when calling via Task tool.
...
| `autopilot` | Full autonomous execution from idea to working code | `/oh-my-black:autopilot` |
| `ultrawork` | Maximum performance with parallel agents | `/oh-my-black:ultrawork` |
| `ultrapilot` | Parallel autopilot with 3-5x speedup | `/oh-my-black:ultrapilot` |
| `swarm` | N coordinated agents with task claiming | `/oh-my-black:swarm` |
| `pipeline` | Sequential agent chaining | `/oh-my-black:pipeline` |
| `ecomode` | Token-efficient parallel execution | `/oh-my-black:ecomode` |
| `ralph` | Self-referential development until completion | `/oh-my-black:ralph` |
| `ralph-init` | Initialize PRD for structured task tracking | `/oh-my-black:ralph-init` |
| `ultraqa` | Autonomous QA cycling workflow | `/oh-my-black:ultraqa` |
| `plan` | Start planning session | `/oh-my-black:plan` |
| `ralplan` | Iterative planning (Planner+Architect+Critic) | `/oh-my-black:ralplan` |
| `review` | Review work plans with critic | `/oh-my-black:review` |

### Enhancement Skills

| Skill | Description | Manual Command |
|-------|-------------|----------------|
| `deepinit` | Hierarchical AGENTS.md codebase documentation | `/oh-my-black:deepinit` |
| `deepsearch` | Thorough multi-strategy codebase search | `/oh-my-black:deepsearch` |
| `analyze` | Deep analysis and investigation | `/oh-my-black:analyze` |
| `research` | Parallel scientist orchestration | `/oh-my-black:research` |
...
| `tdd` | TDD enforcement: test-first development | `/oh-my-black:tdd` |
| `learner` | Extract reusable skill from session | `/oh-my-black:learner` |
| `build-fix` | Fix build and TypeScript errors | `/oh-my-black:build-fix` |
| `code-review` | Comprehensive code review | `/oh-my-black:code-review` |
| `security-review` | Security vulnerability detection | `/oh-my-black:security-review` |

### Utility Skills

| Skill | Description | Manual Command |
|-------|-------------|----------------|
| `note` | Save notes to compaction-resilient notepad | `/oh-my-black:note` |
| `cancel` | Unified cancellation for all modes | `/oh-my-black:cancel` |
| `omb-setup` | One-time setup wizard | `/oh-my-black:omb-setup` |
| `doctor` | Diagnose and fix installation issues | `/oh-my-black:doctor` |
| `help` | Show OMB usage guide | `/oh-my-black:help` |
| `hud` | Configure HUD statusline | `/oh-my-black:hud` |
| `release` | Automated release workflow | `/oh-my-black:release` |
| `mcp-setup` | Configure MCP servers | `/oh-my-black:mcp-setup` |
| `learn-about-omc` | Usage pattern analysis | `/oh-my-black:learn-about-omc` |
| `writer-memory` | Agentic memory system for writers | `/oh-my-black:writer-memory` |
| `project-session-manager` | Manage isolated dev environments (git worktrees + tmux) | `/oh-my-black:project-session-manager` |
| `local-skills-setup` | Set up and manage local skills | `/oh-my-black:local-skills-setup` |
| `skill` | Manage local skills (list, add, remove, search, edit) | `/oh-my-black:skill` |

---

## Slash Commands

All skills are available as slash commands with the prefix `/oh-my-black:`.

| Command | Description |
|---------|-------------|
| `/oh-my-black:orchestrate <task>` | Activate multi-agent orchestration mode |
| `/oh-my-black:autopilot <task>` | Full autonomous execution |
| `/oh-my-black:ultrawork <task>` | Maximum performance mode with parallel agents |
| `/oh-my-black:ultrapilot <task>` | Parallel autopilot (3-5x faster) |
| `/oh-my-black:swarm <N>:<agent> <task>` | Coordinated agent swarm |
| `/oh-my-black:pipeline <stages>` | Sequential agent chaining |
| `/oh-my-black:ecomode <task>` | Token-efficient parallel execution |
| `/oh-my-black:ralph-init <task>` | Initialize PRD for structured task tracking |
| `/oh-my-black:ralph <task>` | Self-referential loop until task completion |
| `/oh-my-black:ultraqa <goal>` | Autonomous QA cycling workflow |
| `/oh-my-black:plan <description>` | Start planning session |
| `/oh-my-black:ralplan <description>` | Iterative planning with consensus |
| `/oh-my-black:review [plan-path]` | Review a plan with critic |
| `/oh-my-black:deepsearch <query>` | Thorough multi-strategy codebase search |
| `/oh-my-black:deepinit [path]` | Index codebase with hierarchical AGENTS.md files |
| `/oh-my-black:analyze <target>` | Deep analysis and investigation |
| `/oh-my-black:research <topic>` | Parallel research orchestration |
| `/oh-my-black:tdd <feature>` | TDD workflow enforcement |
| `/oh-my-black:learner` | Extract reusable skill from session |
| `/oh-my-black:note <content>` | Save notes to notepad.md |
| `/oh-my-black:cancel` | Unified cancellation |
| `/oh-my-black:omb-setup` | One-time setup wizard |
| `/oh-my-black:doctor` | Diagnose and fix installation issues |
| `/oh-my-black:help` | Show OMB usage guide |
| `/oh-my-black:hud` | Configure HUD statusline |
| `/oh-my-black:release` | Automated release workflow |
| `/oh-my-black:mcp-setup` | Configure MCP servers |
...
> **Advanced**: Set `OMB_USE_NODE_HOOKS=1` to use Node.js hooks on macOS/Linux.
...
oh-my-black includes comprehensive monitoring for agent performance, token usage, and debugging parallel workflows.
...
| **Session Replay** | Event timeline for post-session analysis | `.omb/state/agent-replay-*.jsonl` |
...
| Issue | Solution |
|-------|----------|
| Commands not found | Re-run `/oh-my-black:omb-setup` |
...
| Token limit errors | Use `/oh-my-black:ecomode` for token-efficient execution |
...
```bash
curl -fsSL https://raw.githubusercontent.com/black940514/oh-my-black/main/scripts/uninstall.sh | bash
```
