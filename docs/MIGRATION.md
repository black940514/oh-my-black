...
This guide covers all migration paths for oh-my-black. Find your current version below.
...
8 deprecated skills have been removed. The unified `/cancel` and `/omb-setup` commands replace them.
...
|---------------|-------------|
| `cancel-autopilot` | `/oh-my-black:cancel` |
| `cancel-ralph` | `/oh-my-black:cancel` |
| `cancel-ultrawork` | `/oh-my-black:cancel` |
| `cancel-ultraqa` | `/oh-my-black:cancel` |
| `cancel-ecomode` | `/oh-my-black:cancel` |
| `omc-default` | `/oh-my-black:omb-setup --local` |
| `omc-default-global` | `/oh-my-black:omb-setup --global` |
| `planner` | `/oh-my-black:plan` |

### What Changed

**Before v3.5.3:**
```bash
/oh-my-black:cancel-ralph      # Cancel ralph specifically
/oh-my-black:omc-default       # Configure local project
/oh-my-black:planner "task"    # Start planning
```

**After v3.5.3:**
```bash
/oh-my-black:cancel            # Auto-detects and cancels any active mode
/oh-my-black:omb-setup --local # Configure local project
/oh-my-black:plan "task"       # Start planning (includes interview mode)
```

### New Features

**New skill: `/learn-about-omc`**
- Analyzes your OMB usage patterns
- Provides personalized recommendations
- Identifies underutilized features

**Plan skill now supports consensus mode:**
```bash
/oh-my-black:plan --consensus "task"  # Iterative planning with Critic review
/oh-my-black:ralplan "task"           # Alias for plan --consensus
```
...
3. **Re-run `/omb-setup`** if you want to update your CLAUDE.md configuration
...
**Before 3.0:** Explicitly invoke 25+ commands like `/oh-my-black:ralph "task"`, `/oh-my-black:ultrawork "task"`
...
/oh-my-black:ralph "implement user authentication"       # Persistence mode
/oh-my-black:ultrawork "refactor the API layer"          # Maximum parallelism
/oh-my-black:planner "plan the new dashboard"            # Planning interview
/oh-my-black:deepsearch "find database schema files"     # Deep search
/oh-my-black:git-master "commit these changes"           # Git expertise
/oh-my-black:deepinit ./src                              # Index codebase
/oh-my-black:analyze "why is this test failing?"         # Deep analysis
```
...
All environment variables have been renamed from `SISYPHUS_*` to `OMB_*`:

| Old | New |
|-----|-----|
| SISYPHUS_USE_NODE_HOOKS | OMB_USE_NODE_HOOKS |
| SISYPHUS_USE_BASH_HOOKS | OMB_USE_BASH_HOOKS |
| SISYPHUS_PARALLEL_EXECUTION | OMB_PARALLEL_EXECUTION |
| SISYPHUS_LSP_TOOLS | OMB_LSP_TOOLS |
| SISYPHUS_MAX_BACKGROUND_TASKS | OMB_MAX_BACKGROUND_TASKS |
| SISYPHUS_ROUTING_ENABLED | OMB_ROUTING_ENABLED |
| SISYPHUS_ROUTING_DEFAULT_TIER | OMB_ROUTING_DEFAULT_TIER |
| SISYPHUS_ESCALATION_ENABLED | OMB_ESCALATION_ENABLED |
| SISYPHUS_DEBUG | OMB_DEBUG |
...
|-------------|----------------|--------|
| `/oh-my-black:ralph "task"` | Say "don't stop until done" OR use `ralph` keyword | ✅ YES (both ways) |
| `/oh-my-black:ultrawork "task"` | Say "fast" or "parallel" OR use `ulw` keyword | ✅ YES (both ways) |
| `/oh-my-black:ultrawork-ralph` | Say "ralph ulw:" prefix | ✅ YES (keyword combo) |
| `/oh-my-black:planner "task"` | Say "plan this" OR use `plan` keyword | ✅ YES (both ways) |
| `/oh-my-black:plan "description"` | Start planning naturally | ✅ YES |
| `/oh-my-black:review [path]` | Invoke normally | ✅ YES (unchanged) |
| `/oh-my-black:deepsearch "query"` | Say "find" or "search" | ✅ YES (auto-detect) |
| `/oh-my-black:analyze "target"` | Say "analyze" or "investigate" | ✅ YES (auto-detect) |
| `/oh-my-black:deepinit [path]` | Invoke normally | ✅ YES (unchanged) |
| `/oh-my-black:git-master` | Say "git", "commit", "atomic commit" | ✅ YES (auto-detect) |
| `/oh-my-black:frontend-ui-ux` | Say "UI", "styling", "component", "design" | ✅ YES (auto-detect) |
| `/oh-my-black:note "content"` | Say "remember this" or "save this" | ✅ YES (auto-detect) |
| `/oh-my-black:cancel-ralph` | Say "stop", "cancel", or "abort" | ✅ YES (auto-detect) |
| `/oh-my-black:doctor` | Invoke normally | ✅ YES (unchanged) |
...
No more `/oh-my-black:cancel-ralph` - just say "cancel"!
...
2. **Verify directories exist**:
   ```bash
   ls -la .omb/  # In project directory
   ls -la ~/.omb/  # Global directory
   ```

3. **Test a simple command**:
   Run `/oh-my-black:help` in Claude Code to ensure the plugin is loaded correctly.
...
```
Before: "OK, I need to use /oh-my-black:ultrawork for speed..."
...
#### 3. Learned Skills (`/oh-my-black:learner`)
...
Storage:
- **Project-level**: `.omb/skills/` (version-controlled)
...
```
[OMB] ralph:3/10 | US-002 | ultrawork skill:planner | ctx:67% | agents:2 | todos:2/5
```

Run `/oh-my-black:hud setup` to install. Presets: minimal, focused, full.
...
Or use `/oh-my-black:note` to save discoveries manually:

```bash
/oh-my-black:note Project uses PostgreSQL with Prisma ORM
```
...
```bash
/oh-my-black:ralph-init "implement OAuth with multiple providers"
    ↓
...
```bash
/oh-my-black:ultrapilot "build a fullstack todo app"
```
...
- State files: `.omb/state/ultrapilot-state.json`, `.omb/state/ultrapilot-ownership.json`
...
```bash
/oh-my-black:swarm 5:executor "fix all TypeScript errors"
```
...
```bash
/oh-my-black:pipeline explore:haiku -> architect:opus -> executor:sonnet
```
...
```bash
/oh-my-black:ecomode "refactor the authentication system"
```
...
- `/oh-my-black:cancel-ralph` (deprecated)
- `/oh-my-black:cancel-ultraqa` (deprecated)
- `/oh-my-black:cancel-ultrawork` (deprecated)
- `/oh-my-black:cancel-ecomode` (deprecated)
- `/oh-my-black:cancel-autopilot` (deprecated)

Use `/oh-my-black:cancel` instead.
...
```typescript
Task(subagent_type="oh-my-black:explore-high",
     model="opus",
...
**Standard paths:**
- Local: `.omb/state/{name}.json`
- Global: `~/.omb/state/{name}.json`
...
Users set their default mode preference via `/oh-my-black:omb-setup`.
...
Set your preferred execution mode in `~/.claude/.omb-config.json`:
...
2. **Test ultrapilot**:
   ```bash
   /oh-my-black:ultrapilot "create a simple React component"
   ```
...
4. **Check state directory**:
   ```bash
   ls -la .omb/state/  # Should see ultrapilot-state.json after running ultrapilot
   ```
...
- Watch the [GitHub repository](https://github.com/black940514/oh-my-black) for announcements
...
**2.x Workflow:**
```
/oh-my-black:ultrawork "implement the todo list feature"
```
...
**2.x Workflow:**
```
/oh-my-black:ralph "debug the memory leak"
```
...
**2.x Workflow:**
```
/oh-my-black:planner "design the new authentication system"
```
...
Apply oh-my-black to current project only:

```
/oh-my-black:omc-default
```
...
```
/oh-my-black:omc-default-global
```
...
A: Keep using them! `/oh-my-black:ralph`, `/oh-my-black:ultrawork`, and `/oh-my-black:plan` work. Note: `/oh-my-black:planner` now redirects to `/oh-my-black:plan`.

**Q: How do I know what Claude is doing?**
A: Claude announces major behaviors: "I'm activating ralph-loop..." or set up `/oh-my-black:hud` for real-time status.
...
- **Diagnose issues**: Run `/oh-my-black:doctor`
- **See all commands**: Run `/oh-my-black:help`
- **View real-time status**: Run `/oh-my-black:hud setup`
- **Review detailed changelog**: See [CHANGELOG.md](../CHANGELOG.md)
- **Report bugs**: [GitHub Issues](https://github.com/black940514/oh-my-black/issues)
