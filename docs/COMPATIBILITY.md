...
The Compatibility Layer enables oh-my-black to discover, register, and use external plugins, MCP servers, and tools. It provides a unified interface for managing external tools while maintaining security through an integrated permission system.
...
3. **Permission Adapter** - Integrates with OMB's permission system for safe tool execution
4. **MCP Bridge** - Connects to MCP servers and exposes their tools for use

```
Plugins              MCP Configs          OMB Tools
   ↓                      ↓                    ↓
 Discovery System ────────────────────────────┐
                                              ↓
                           Tool Registry ← ← ←┘
...
- `~/.claude/plugins/` - OMB/Claude Code plugins directory
...
Integrates external tools with OMB's permission system:
...
- Plugin name doesn't conflict with reserved names (e.g., 'oh-my-black')
