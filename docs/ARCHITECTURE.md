...
> How oh-my-black orchestrates multi-agent workflows.

## Overview

oh-my-black enables Claude Code to orchestrate specialized agents through a skill-based routing system.
...
**Local Project State:**
- `.omb/state/{name}.json` - Session state (ultrapilot, swarm, pipeline)
- `.omb/notepads/{plan-name}/` - Plan-scoped wisdom capture

**Global State:**
- `~/.omb/state/{name}.json` - User preferences and global config

Legacy locations are auto-migrated on read.

## Hooks

oh-my-black includes 31 hooks in `src/hooks/` for lifecycle events:
