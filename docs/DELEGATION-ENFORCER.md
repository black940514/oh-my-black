...
```typescript
import { enforceModel } from 'oh-my-black';
...
```typescript
import { getModelForAgent } from 'oh-my-black';
...
```typescript
import { isAgentCall } from 'oh-my-black';
...
```typescript
import { processHook } from 'oh-my-black';
...
```bash
export OMB_DEBUG=true
```

When enabled, you'll see warnings like:

```
[OMB] Auto-injecting model: sonnet for executor
```

**Important:** Warnings are ONLY shown when `OMB_DEBUG=true`. Without this flag, enforcement happens silently.
