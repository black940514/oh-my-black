...
Comprehensive guide to monitoring, debugging, and optimizing Claude Code and oh-my-black performance.
...
oh-my-black provides comprehensive monitoring capabilities for tracking agent performance, token usage, costs, and identifying bottlenecks in multi-agent workflows. This guide covers both built-in tools and external resources for monitoring Claude's performance.
...
```typescript
import { getAgentObservatory } from 'oh-my-black/hooks/subagent-tracker';
...
OMB automatically tracks token usage and costs across all sessions.
...
Replay data is stored at: `.omb/state/agent-replay-{sessionId}.jsonl`
...
```typescript
import { getReplaySummary } from 'oh-my-black/hooks/subagent-tracker/session-replay';
...
```typescript
import { getAgentPerformance } from 'oh-my-black/hooks/subagent-tracker';
...
```typescript
import { detectFileConflicts } from 'oh-my-black/hooks/subagent-tracker';
...
OMB automatically detects problematic agents:
...
```typescript
import { suggestInterventions } from 'oh-my-black/hooks/subagent-tracker';
...
```typescript
import { calculateParallelEfficiency } from 'oh-my-black/hooks/subagent-tracker';
...
```typescript
import { cleanupStaleAgents } from 'oh-my-black/hooks/subagent-tracker';
...
| OMB GitHub Issues | Bug reports and feature requests | [GitHub Issues](https://github.com/black940514/oh-my-black/issues) |
...
```bash
# Find replay files
ls .omb/state/agent-replay-*.jsonl

# View recent events
tail -20 .omb/state/agent-replay-*.jsonl
```
...
```typescript
import { cleanupReplayFiles } from 'oh-my-black/hooks/subagent-tracker/session-replay';
...
2. Verify HUD is running: `/oh-my-black:hud setup`
3. Check `.omb/state/` directory exists
...
2. Delete `.omb/state/subagent-tracking.json` to reset
3. Check for orphaned lock files: `.omb/state/subagent-tracker.lock`
...
| File | Purpose | Format |
|------|---------|--------|
| `.omb/state/subagent-tracking.json` | Current agent states | JSON |
| `.omb/state/agent-replay-{id}.jsonl` | Session event timeline | JSONL |
| `.omb/state/token-tracking.jsonl` | Token usage log | JSONL |
| `.omb/state/analytics-summary-{id}.json` | Cached session summaries | JSON |
| `.omb/state/subagent-tracker.lock` | Concurrent access lock | Text |
