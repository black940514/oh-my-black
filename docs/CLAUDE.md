<!-- OMB:START -->
# oh-my-black - Intelligent Multi-Agent Orchestration
...
Users set their preference via `/oh-my-black:omb-setup`.
...
```
Task(subagent_type="oh-my-black:executor",
     model="sonnet",
     prompt="Edit src/file.ts to add validation...")
```

This is **soft enforcement** (warnings only). Audit log at `.omb/logs/delegation-audit.jsonl`.
...
**Setup:** `omb-setup`, `mcp-setup`, `hud`, `doctor`, `help`

Run `/oh-my-black:help` for the complete skill reference with triggers.
...
### All 38 Agents (33 + 5 Validators)

See [Agent Tiers Reference](./shared/agent-tiers.md) for the complete agent tier matrix with all agents organized by domain and tier.

**New validators (ohmyblack):**
- `validator-syntax` (haiku) - Type/syntax verification
- `validator-logic` (sonnet) - Functional verification
- `validator-security` (opus) - Security analysis
- `validator-integration` (sonnet) - Cross-component verification
- `coordinator` (sonnet) - Builder-Validator cycle management
...
ohmyblack is an enhanced orchestration system that augments oh-my-black with verification-driven workflows and intelligent team composition. See [Migration Guide: ohmyblack](./MIGRATION-OHMYBLACK.md) for complete documentation.
...
- **Validator Agents** - Specialized validators for syntax, logic, security, and integration
- **DAG-Based Workflow** - Parallel execution with intelligent dependency management

#### Activation

| Mode | Flag | Description |
|------|------|-------------|
| Autopilot | `autopilot --ohmyblack` | Full autonomous with B-V cycles |
| Ralph | `ralph --ohmyblack` | Persistent with B-V cycles |
| Ultrawork | `ultrawork --auto-team` | Parallel with auto team composition |
| Global | `export OMB_OHMYBLACK_DEFAULT=true` | Enable for all commands |

#### Validation Levels

| Level | Validators | Use Case | Overhead |
|-------|-----------|----------|----------|
| self-only | None | Quick, low-risk tasks | +5% |
| validator | syntax, logic | Standard work | +15% |
| architect | All + architect review | Critical changes | +30% |

**Set per command:**
```bash
autopilot --ohmyblack --validation=validator "Your task"
```
...
| fullstack | >= 0.9 | Multiple builders + full team | architect |

**Auto-select team by complexity:**
```bash
ultrawork --auto-team "Your task"
```

#### New Validator Agents

- **validator-syntax** (haiku) - Type/syntax verification
- **validator-logic** (sonnet) - Functional requirement verification
- **validator-security** (opus) - Security vulnerability checks
- **validator-integration** (sonnet) - Cross-component verification
- **coordinator** (sonnet) - Builder-Validator cycle management

#### Configuration

Add to `.omb-config.json`:
...
| Path | Purpose |
|------|---------|
| `.omb/state/teams/*.json` | Team definitions |
| `.omb/state/workflows/*.json` | Workflow state |
| `.omb/state/validations/*.json` | B-V cycle results |
| `.omb/state/coordination/*.json` | Coordinator state |

#### Backward Compatibility

ohmyblack is **fully backward compatible with v3.9.5**. All existing workflows continue working unchanged. Enable features using `--ohmyblack` flag or configuration.
...
The notepad at `.omb/notepad.md` provides compaction-resilient memory with three tiers:
...
**Usage via `/oh-my-black:note` skill:**

```
/oh-my-black:note <content>              # Add to Working Memory
/oh-my-black:note --priority <content>   # Add to Priority Context (always loaded)
/oh-my-black:note --manual <content>     # Add to MANUAL (never pruned)
/oh-my-black:note --show                 # Display notepad contents
/oh-my-black:note --prune                # Remove entries older than 7 days
/oh-my-black:note --clear                # Clear Working Memory only
```
...
Say "setup omc" or run `/oh-my-black:omb-setup` to configure. After that, everything is automatic.

### Troubleshooting

- `/oh-my-black:doctor` - Diagnose and fix installation issues
- `/oh-my-black:hud setup` - Install/repair HUD statusline
