...
Compaction-resilient memory system at `.omb/notepad.md` with three tiers:
...
**User skill:** `/oh-my-black:note`
- `/oh-my-black:note <content>` - Add to Working Memory
- `/oh-my-black:note --priority <content>` - Add to Priority Context
- `/oh-my-black:note --manual <content>` - Add to MANUAL section
...
**State files:**
- `.omb/state/ultrapilot-state.json` - Session state
...
- Primary: `.omb/state/{name}.json` (local, per-project)
- Global backup: `~/.omb/state/{name}.json` (global, session continuity)
...
**Important:** Never store OMB state in `~/.claude/` - that directory is reserved for Claude Code itself.
