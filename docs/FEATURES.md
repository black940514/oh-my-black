...
> Internal API documentation for oh-my-black developers and contributors.
...
Plan-scoped knowledge capture for agents executing tasks. Each plan gets its own notepad directory at `.omb/notepads/{plan-name}/` with four markdown files:
...
// Paths
getSpecPath(directory: string): string  // .omb/autopilot/spec.md
getPlanPath(directory: string): string  // .omb/plans/autopilot-impl.md
```
...
All state is persisted to `.omb/state/autopilot-state.json` and includes:
