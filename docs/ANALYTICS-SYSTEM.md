# OMB Analytics System

## Overview

Automatic token tracking and cost estimation for Claude API usage in oh-my-black.
...
- `.omb/state/token-tracking.jsonl` - Append-only token log
- `.omb/state/analytics-summary-{sessionId}.json` - Cached summaries
