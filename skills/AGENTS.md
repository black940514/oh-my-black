...
Skills are reusable workflow templates that can be invoked via `/oh-my-black:skill-name`. Each skill provides:
...
| `cancel/SKILL.md` | cancel | Cancel any active OMB mode |
| `hud/SKILL.md` | hud | Configure HUD display |
| `doctor/SKILL.md` | doctor | Diagnose installation issues |
| `omb-setup/SKILL.md` | omb-setup | One-time setup wizard |
...
```bash
# Manual invocation
/oh-my-black:skill-name

# With arguments
/oh-my-black:skill-name arg1 arg2
...
- Test skill invocation with `/oh-my-black:skill-name`
...
| Utility | learner, note, cancel, hud, doctor, omb-setup, mcp-setup, help | "stop", "cancel" |
