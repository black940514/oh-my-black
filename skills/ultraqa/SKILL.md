...
|------------|-----------|---------------|
| `/oh-my-black:ultraqa --tests` | tests | All test suites pass |
| `/oh-my-black:ultraqa --build` | build | Build succeeds with exit 0 |
| `/oh-my-black:ultraqa --lint` | lint | No lint errors |
| `/oh-my-black:ultraqa --typecheck` | typecheck | No TypeScript errors |
| `/oh-my-black:ultraqa --custom "pattern"` | custom | Custom success pattern in output |
...
1. **RUN QA**: Execute verification based on goal type
   ...
   - `--interactive`: Use qa-tester for interactive CLI/service testing:
     ```
     Task(subagent_type="oh-my-black:qa-tester", model="sonnet", prompt="TEST:
...
3. **ARCHITECT DIAGNOSIS**: Spawn architect to analyze failure
   ```
   Task(subagent_type="oh-my-black:architect", model="opus", prompt="DIAGNOSE FAILURE:
   Goal: [goal type]
...
4. **FIX ISSUES**: Apply architect's recommendations
   ```
   Task(subagent_type="oh-my-black:executor", model="sonnet", prompt="FIX:
...
Track state in `.omb/ultraqa-state.json`:
```json
{
  "active": true,
  "goal_type": "tests",
...
User can cancel with `/oh-my-black:cancel` which clears the state file.
