...
Connect to swarm at ${cwd}/.omb/state/swarm.db
...
User can cancel via `/oh-my-black:cancel`:
...
rm -f .omb/state/swarm-state.json
rm -f .omb/state/swarm-tasks.json
rm -f .omb/state/swarm-claims.json
```
