...
description: Analyze your OMB usage patterns and get personalized recommendations
---

# Learn About OMB

Analyzes your oh-my-black usage and provides tailored recommendations to improve your workflow.

## What It Does

1. Reads token tracking from `~/.omb/state/token-tracking.jsonl`
2. Reads session history from `.omb/state/session-history.json`
...
TOKEN_FILE="$HOME/.omb/state/token-tracking.jsonl"
SESSION_FILE=".omb/state/session-history.json"
CONFIG_FILE="$HOME/.claude/.omb-config.json"

echo "📊 Analyzing OMB Usage..."
echo ""
...
- "Set defaultExecutionMode in /omb-setup for consistent behavior"
...
1. Ensure ~/.omb/state/ directory exists
2. Run any OMB command to start tracking

Tip: Run /omb-setup to configure OMB properly.
```

## Example Output

```
📊 Your OMB Usage Analysis
