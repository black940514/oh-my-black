...
Configure the OMB HUD (Heads-Up Display) for the statusline.

## Quick Commands

| Command | Description |
|---------|-------------|
| `/oh-my-black:hud` | Show current HUD status (auto-setup if needed) |
| `/oh-my-black:hud setup` | Install/repair HUD statusline |
| `/oh-my-black:hud minimal` | Switch to minimal display |
| `/oh-my-black:hud focused` | Switch to focused display (default) |
| `/oh-my-black:hud full` | Switch to full display |
| `/oh-my-black:hud status` | Show detailed HUD status |

## Auto-Setup

When you run `/oh-my-black:hud` or `/oh-my-black:hud setup`, the system will automatically:
...
PLUGIN_VERSION=$(ls ~/.claude/plugins/cache/omc/oh-my-black/ 2>/dev/null | sort -V | tail -1)
if [ -n "$PLUGIN_VERSION" ]; then
  ls ~/.claude/plugins/cache/omc/oh-my-black/$PLUGIN_VERSION/dist/hud/index.js 2>/dev/null && echo "BUILT" || echo "NOT_BUILT"
fi
```
...
```bash
cd ~/.claude/plugins/cache/omc/oh-my-black/$PLUGIN_VERSION && npm install
```
...
async function main() {
  ...
  if (pluginCacheDir) {
    console.log(`[OMB] HUD not built. Run: cd "${pluginCacheDir}" && npm install`);
  } else {
    console.log("[OMB] Plugin not found. Run: /oh-my-black:omb-setup");
  }
}
...
```
[OMB] ralph | ultrawork | todos:2/5
```

### Focused (Default)
Shows all relevant elements:
```
[OMB] ralph:3/10 | US-002 | ultrawork skill:planner | ctx:67% | agents:2 | bg:3/5 | todos:2/5
```

### Full
Shows everything including multi-line agent details:
```
[OMB] ralph:3/10 | US-002 (2/5) | ultrawork | ctx:[████░░]67% | agents:3 | bg:3/5 | todos:2/5
...
HUD config is stored at: `~/.claude/.omb/hud-config.json`
...
1. Run `/oh-my-black:hud setup` to auto-install and configure
2. Restart Claude Code after setup completes
3. If still not working, run `/oh-my-black:doctor` for full diagnostics
