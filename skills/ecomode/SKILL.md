...
Task(subagent_type="oh-my-black:explore", model="haiku", prompt="...")
...
Task(subagent_type="oh-my-black:architect-medium", model="sonnet", prompt="...")

// AVOID - Only for planning/critique if essential
Task(subagent_type="oh-my-black:planner", model="opus", prompt="...")
```
...
Ecomode state is tracked in `.omb/state/ecomode-state.json`.

When work is complete, run `/oh-my-black:cancel` for clean state cleanup.
