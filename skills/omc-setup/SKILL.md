---
name: omb-setup
description: Setup and configure oh-my-black (the ONLY command you need to learn)
---
...
CONFIG_FILE="$HOME/.claude/.omb-config.json"
...
if [ -f "$CONFIG_FILE" ]; then
  ...
  if [ -n "$SETUP_COMPLETED" ] && [ "$SETUP_COMPLETED" != "null" ]; then
    echo "OMB setup was already completed on: $SETUP_COMPLETED"
...
**Question:** "OMB is already configured. What would you like to do?"
...
### State File Location
- `.omb/state/setup-state.json` - Tracks completed steps
...
STATE_FILE=".omb/state/setup-state.json"
...
```bash
rm -f ".omb/state/setup-state.json"
...
save_setup_progress() {
  mkdir -p .omb/state
  cat > ".omb/state/setup-state.json" << EOF
{
  "lastCompletedStep": $1,
...
**Question:** "Where should I configure oh-my-black?"
...
OLD_VERSION=$(grep -m1 "^# oh-my-black" "$TARGET_PATH" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "none")
...
# Download fresh OMB content to temp file
TEMP_OMB=$(mktemp /tmp/omc-claude-XXXXXX.md)
trap 'rm -f "$TEMP_OMB"' EXIT
curl -fsSL "https://raw.githubusercontent.com/black940514/oh-my-black/main/docs/CLAUDE.md" -o "$TEMP_OMB"
...
if grep -q '<!-- OMB:START -->' "$TEMP_OMB"; then
  # Extract content between markers
  sed -n '/<!-- OMB:START -->/,/<!-- OMB:END -->/{//!p}' "$TEMP_OMB" > "${TEMP_OMB}.clean"
  mv "${TEMP_OMB}.clean" "$TEMP_OMB"
fi
...
if [ ! -f "$TARGET_PATH" ]; then
  ...
  # Merge: preserve user content outside OMB markers
  if grep -q '<!-- OMB:START -->' "$TARGET_PATH"; then
    # Has markers: replace OMB section, keep user content
    BEFORE_OMB=$(sed -n '1,/<!-- OMB:START -->/{ /<!-- OMB:START -->/!p }' "$TARGET_PATH")
    AFTER_OMB=$(sed -n '/<!-- OMB:END -->/,${  /<!-- OMB:END -->/!p }' "$TARGET_PATH")
    {
      [ -n "$BEFORE_OMB" ] && printf '%s\n' "$BEFORE_OMB"
      echo '<!-- OMB:START -->'
      cat "$TEMP_OMB"
      echo '<!-- OMB:END -->'
      [ -n "$AFTER_OMB" ] && printf '%s\n' "$AFTER_OMB"
    } > "${TARGET_PATH}.tmp"
    mv "${TARGET_PATH}.tmp" "$TARGET_PATH"
    echo "Updated OMB section (user customizations preserved)"
  else
    ...
    mv "${TARGET_PATH}.tmp" "$TARGET_PATH"
    echo "Migrated existing CLAUDE.md (added OMB markers, preserved old content)"
  fi
  rm -f "$TEMP_OMB"
fi

# Extract new version and report
NEW_VERSION=$(grep -m1 "^# oh-my-black" "$TARGET_PATH" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
...
https://raw.githubusercontent.com/black940514/oh-my-black/main/docs/CLAUDE.md

### Verify Plugin Installation

```bash
grep -q "oh-my-black" ~/.claude/settings.json && echo "Plugin verified" || echo "Plugin NOT found - run: claude /install-plugin oh-my-black"
```
...
**OMB Project Configuration Complete**
...
**OMB Global Configuration Complete**
- CLAUDE.md: Updated with latest configuration from GitHub at ~/.claude/CLAUDE.md
- Backup: Previous CLAUDE.md backed up to `~/.claude/CLAUDE.md.backup.YYYY-MM-DD` (if existed)
...
mkdir -p .omb/state
CONFIG_TYPE=$(cat ".omb/state/setup-state.json" 2>/dev/null | grep -oE '"configType":\s*"[^"]+"' | cut -d'"' -f4 || echo "unknown")
cat > ".omb/state/setup-state.json" << EOF
{
  "lastCompletedStep": 3,
  "timestamp": "$(date -Iseconds)",
...
CACHE_DIR="$HOME/.claude/plugins/cache/omc/oh-my-black"
if [ -d "$CACHE_DIR" ]; then
...
if [ -d "$HOME/.claude/plugins/cache/omc/oh-my-black" ]; then
  INSTALLED_VERSION=$(ls -1 "$HOME/.claude/plugins/cache/omc/oh-my-black" | sort -V | tail -1)
fi
...
if [ -z "$INSTALLED_VERSION" ]; then
  if [ -f ".claude/CLAUDE.md" ]; then
    INSTALLED_VERSION=$(grep -m1 "^# oh-my-black" .claude/CLAUDE.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^v//')
  elif [ -f "$HOME/.claude/CLAUDE.md" ]; then
    INSTALLED_VERSION=$(grep -m1 "^# oh-my-black" "$HOME/.claude/CLAUDE.md" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' | sed 's/^v//')
  fi
fi

# Check npm for latest version
LATEST_VERSION=$(npm view oh-my-black version 2>/dev/null)
...
if [ -n "$INSTALLED_VERSION" ] && [ -n "$LATEST_VERSION" ]; then
  ...
  if [ "$INSTALLED_VERSION" != "$LATEST_VERSION" ]; then
    ...
    echo ""
    echo "To update, run: claude /install-plugin oh-my-black"
  else
...
Store the preference in `~/.claude/.omb-config.json`:
...
The OMB CLI provides standalone token analytics commands (`omc stats`, `omc agents`, `omc tui`).

Ask user: "Would you like to install the OMB CLI for standalone analytics? (Recommended for tracking token usage and costs)"
...
- Use `/oh-my-black:help` for guidance
- Use `/oh-my-black:doctor` for diagnostics
...
MCP SERVERS:
Run /oh-my-black:mcp-setup to add tools like web search, GitHub, etc.

HUD STATUSLINE:
The status bar now shows OMB state. Restart Claude Code to see it.
...
```
OMB Setup Complete! (Upgraded from 2.x)
...
**Question:** "If you're enjoying oh-my-black, would you like to support the project by starring it on GitHub?"
...
```bash
gh api -X PUT /user/starred/black940514/oh-my-black 2>/dev/null && echo "Thanks for starring! ⭐" || true
```
...
```bash
echo ""
echo "If you enjoy oh-my-black, consider starring the repo:"
echo "  https://github.com/black940514/oh-my-black"
echo ""
```
...
# Get current OMB version from CLAUDE.md
OMB_VERSION=""
if [ -f ".claude/CLAUDE.md" ]; then
  OMB_VERSION=$(grep -m1 "^# oh-my-black" .claude/CLAUDE.md 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
elif [ -f "$HOME/.claude/CLAUDE.md" ]; then
  OMB_VERSION=$(grep -m1 "^# oh-my-black" "$HOME/.claude/CLAUDE.md" 2>/dev/null | grep -oE 'v[0-9]+\.[0-9]+\.[0-9]+' || echo "unknown")
fi
...
echo "$EXISTING" | jq --arg ts "$(date -Iseconds)" --arg ver "$OMB_VERSION" \
...
After installing oh-my-black updates (via npm or plugin update):

**Automatic**: Just run `/oh-my-black:omb-setup` - it will detect you've already configured and offer a quick "Update CLAUDE.md only" option that skips the full wizard.

**Manual options**:
- `/oh-my-black:omb-setup --local` to update project config only
- `/oh-my-black:omb-setup --global` to update global config only
- `/oh-my-black:omb-setup --force` to re-run the full wizard (reconfigure preferences)

This ensures you have the newest features and agent configurations without the token cost of repeating the full setup.

## Help Text

When user runs `/oh-my-black:omb-setup --help` or just `--help`, display:

```
OMB Setup - Configure oh-my-black

USAGE:
  /oh-my-black:omb-setup           Run initial setup wizard (or update if already configured)
  /oh-my-black:omb-setup --local   Configure local project (.claude/CLAUDE.md)
  /oh-my-black:omb-setup --global  Configure global settings (~/.claude/CLAUDE.md)
  /oh-my-black:omb-setup --force   Force full setup wizard even if already configured
  /oh-my-black:omb-setup --help    Show this help
...
MODES:
  ...
  Local Configuration (--local)
    ...
    - Use this to update project config after OMB upgrades
  ...
  Global Configuration (--global)
    ...
    - Use this to update global config after OMB upgrades
...
EXAMPLES:
  /oh-my-black:omb-setup           # First time setup (or update CLAUDE.md if configured)
  /oh-my-black:omb-setup --local   # Update this project
  /oh-my-black:omb-setup --global  # Update all projects
  /oh-my-black:omb-setup --force   # Re-run full setup wizard

For more info: https://github.com/black940514/oh-my-black
```
