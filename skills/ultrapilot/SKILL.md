...
```
/oh-my-black:ultrapilot <your task>
/oh-my-black:up "Build a full-stack todo app"
/oh-my-black:ultrapilot Refactor the entire backend
```
...
const response = await Task({
  subagent_type: 'oh-my-black:architect',
  model: 'opus',
  prompt
});
...
**Data Structure:** `.omb/state/ultrapilot-ownership.json`
...
**Location:** `.omb/ultrapilot-state.json`
...
**Location:** `.omb/state/ultrapilot-ownership.json`

Tracks which worker owns which files (see Phase 2 example above).

### Progress Tracking

**Location:** `.omb/ultrapilot/progress.json`
...
```
/oh-my-black:ultrapilot Build a todo app with React frontend, Express backend, and PostgreSQL database
```
...
```
/oh-my-black:up Refactor all services to use dependency injection
```
...
```
/oh-my-black:ultrapilot Generate tests for all untested modules
```
...
4. **Monitor progress** - Check `.omb/ultrapilot/progress.json`
...
- Review `.omb/ultrapilot/decomposition.json` for details

**Worker hangs?**
- Check worker logs in `.omb/logs/ultrapilot-worker-N.log`
- Cancel and restart that worker
- May indicate file ownership issue

**Integration conflicts?**
- Review `.omb/ultrapilot-state.json` conflicts array
...
**Location:** `.omb/ultrapilot/custom-decomposition.json`
...
Then run:
```
/oh-my-black:ultrapilot --custom-decomposition
```
