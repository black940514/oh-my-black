...
<Builder_Validator_Integration>

## Builder-Validator Integration

The orchestrate skill supports optional Builder-Validator (B-V) cycle verification for tasks that require higher confidence in correctness.

### Enabling B-V Cycle

Tasks can be executed with Builder-Validator verification by specifying validation level:

```
orchestrate --validation=validator task1 task2
orchestrate --validation=architect "complex refactoring task"
```

### Validation Levels

| Level | Description | Validators Used | Best For |
|-------|-------------|-----------------|----------|
| `self-only` | Builder self-validation only | None | Simple, low-risk changes |
| `validator` | Syntax + Logic validation | validator-syntax, validator-logic | Standard feature work |
| `architect` | Full validation suite | All validators + architect review | Complex, high-risk changes |

### How B-V Cycle Works

1. **Builder Phase**: A builder agent executes the task
2. **Self-Validation**: Builder runs internal checks (diagnostics, tests)
3. **External Validation**: If enabled, validator agents review the work
4. **Retry Loop**: If validation fails, builder retries with feedback
5. **Escalation**: After max retries, escalates to coordinator/architect/human
...
- **Escalation Triggers**:
  ...
  - Non-retryable errors (missing dependencies, architectural issues)
  - Persistent issues across 3+ attempts
- **Configurable**: `--max-retries=N`

### Coordinator Integration

When B-V cycle is enabled, a Coordinator agent manages:
- Builder agent execution and monitoring
- Validator agent sequencing (syntax -> logic -> security -> integration)
- Retry decision making based on failure analysis
- Escalation handling with context preservation

### Escalation Levels

| Level | Triggered By | Action |
|-------|--------------|--------|
| `coordinator` | Persistent issues after 3 attempts | Re-evaluate approach, delegate to specialist |
| `architect` | Security issues, critical failures | Deep review, architectural guidance |
| `human` | 5+ failures, missing dependencies | Manual intervention required |

### State Tracking

B-V cycle state is tracked in `.omb/state/orchestrate-state.json`:
...
**Simple task (no external validation):**
```
orchestrate "add console.log to debug"
```

**Standard feature with validation:**
```
orchestrate --validation=validator "implement user login API"
```

**Complex refactoring with full validation:**
```
orchestrate --validation=architect --max-retries=5 "migrate database schema"
```

**Multiple tasks with B-V:**
```
orchestrate --validation=validator task1 task2 task3
```

### Integration with Existing Workflows

B-V cycle integrates seamlessly with:
- **ralph**: Persistence loop uses B-V for each iteration
- **ultrawork**: Parallel tasks can each have B-V cycles
- **autopilot**: Full autonomous execution with B-V verification

### API Reference

For programmatic access, use the verification module:
...
import {
  ...
  generateBVReport
} from '@oh-my-black/verification';

// Execute task with B-V
const result = await executeWithBVCycle({
  taskId: 'task-1',
...
} else if (result.escalation?.shouldEscalate) {
  console.log(`Escalate to: ${result.escalation.escalationLevel}`);
}
```
