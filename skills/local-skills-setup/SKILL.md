...
- **Project-level skills** (.omb/skills/) - Version-controlled with your repo
...
echo ""
echo "=== PROJECT-LEVEL SKILLS (.omb/skills/) ==="
if [ -d ".omb/skills" ]; then
  PROJECT_COUNT=$(find ".omb/skills" -name "*.md" 2>/dev/null | wc -l)
  echo "Total skills: $PROJECT_COUNT"

  if [ $PROJECT_COUNT -gt 0 ]; then
    echo ""
    echo "Skills found:"
    find ".omb/skills" -name "*.md" -type f -exec sh -c '
      FILE="$1"
      NAME=$(grep -m1 "^name:" "$FILE" 2>/dev/null | sed "s/name: //")
      DESC=$(grep -m1 "^description:" "$FILE" 2>/dev/null | sed "s/description: //")
      MODIFIED=$(stat -c "%y" "$FILE" 2>/dev/null || stat -f "%Sm" "$FILE" 2>/dev/null)
...
if [ -d ".omb/skills" ]; then
  ...
  find ".omb/skills" -name "*.md" -type f -exec bash -c 'show_skill_details "$0" "project-level"' {} \;
fi
```
...
- **Project-level** (.omb/skills/) - Only for this project
...
- `/oh-my-black:local-skills-setup list` - Show detailed skill inventory
- `/oh-my-black:local-skills-setup add` - Start skill creation (invoke learner)
- `/oh-my-black:local-skills-setup scan` - Scan both skill directories
...
**Version Control**: Project-level skills (.omb/skills/) are committed with your code, so the whole team benefits.
...
- `/oh-my-black:learner` - Extract a skill from current conversation
- `/oh-my-black:note` - Save quick notes (less formal than skills)
- `/oh-my-black:deepinit` - Generate AGENTS.md codebase hierarchy
...
- Run `/oh-my-black:local-skills-setup scan` periodically to review your skill library
