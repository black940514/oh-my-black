---
name: cancel
description: Cancel any active OMB mode (autopilot, ralph, ultrawork, ecomode, ultraqa, swarm, ultrapilot, pipeline)
---

# Cancel Skill

Intelligent cancellation that detects and cancels the active OMB mode.

**The cancel skill is the standard way to complete and exit any OMB mode.**
...
- `.omb/state/autopilot-state.json` → Autopilot detected
- `.omb/state/ralph-state.json` → Ralph detected
- `.omb/state/ultrawork-state.json` → Ultrawork detected
- `.omb/state/ecomode-state.json` → Ecomode detected
- `.omb/state/ultraqa-state.json` → UltraQA detected
- `.omb/state/swarm.db` → Swarm detected (SQLite database)
- `.omb/state/ultrapilot-state.json` → Ultrapilot detected
- `.omb/state/pipeline-state.json` → Pipeline detected
- `.omb/state/plan-consensus.json` → Plan Consensus detected
- `.omb/state/ralplan-state.json` → Plan Consensus detected (legacy)
...
```
/oh-my-black:cancel --force
```

Or use the `--all` alias:

```
/oh-my-black:cancel --all
```

This removes all state files:
- `.omb/state/autopilot-state.json`
- `.omb/state/ralph-state.json`
- `.omb/state/ralph-plan-state.json`
- `.omb/state/ralph-verification.json`
- `.omb/state/ultrawork-state.json`
- `.omb/state/ecomode-state.json`
- `.omb/state/ultraqa-state.json`
- `.omb/state/swarm.db`
- `.omb/state/swarm.db-wal`
- `.omb/state/swarm.db-shm`
- `.omb/state/swarm-active.marker`
- `.omb/state/swarm-tasks.db`
- `.omb/state/ultrapilot-state.json`
- `.omb/state/ultrapilot-ownership.json`
- `.omb/state/pipeline-state.json`
- `.omb/state/plan-consensus.json`
- `.omb/state/ralplan-state.json`
- `.omb/state/boulder.json`
- `.omb/state/hud-state.json`
- `.omb/state/subagent-tracking.json`
- `.omb/state/subagent-tracker.lock`
- `.omb/state/rate-limit-daemon.pid`
- `.omb/state/rate-limit-daemon.log`
- `.omb/state/checkpoints/` (directory)
...
if [[ -f .omb/state/autopilot-state.json ]]; then
  AUTOPILOT_ACTIVE=$(cat .omb/state/autopilot-state.json | jq -r '.active // false')
fi

if [[ -f .omb/state/ralph-state.json ]]; then
  RALPH_ACTIVE=$(cat .omb/state/ralph-state.json | jq -r '.active // false')
fi

if [[ -f .omb/state/ultrawork-state.json ]]; then
  ULTRAWORK_ACTIVE=$(cat .omb/state/ultrawork-state.json | jq -r '.active // false')
fi

if [[ -f .omb/state/ecomode-state.json ]]; then
  ECOMODE_ACTIVE=$(cat .omb/state/ecomode-state.json | jq -r '.active // false')
fi

if [[ -f .omb/state/ultraqa-state.json ]]; then
  ULTRAQA_ACTIVE=$(cat .omb/state/ultraqa-state.json | jq -r '.active // false')
fi

PLAN_CONSENSUS_ACTIVE=false

# Check both new and legacy locations
if [[ -f .omb/state/plan-consensus.json ]]; then
  PLAN_CONSENSUS_ACTIVE=$(cat .omb/state/plan-consensus.json | jq -r '.active // false')
elif [[ -f .omb/state/ralplan-state.json ]]; then
  PLAN_CONSENSUS_ACTIVE=$(cat .omb/state/ralplan-state.json | jq -r '.active // false')
fi
```
...
if [[ "$FORCE_MODE" == "true" ]]; then
  echo "FORCE CLEAR: Removing all OMB state files..."

  # Remove local state files
  rm -f .omb/state/autopilot-state.json
  rm -f .omb/state/ralph-state.json
  rm -f .omb/state/ralph-plan-state.json
  rm -f .omb/state/ralph-verification.json
  rm -f .omb/state/ultrawork-state.json
  rm -f .omb/state/ecomode-state.json
  rm -f .omb/state/ultraqa-state.json
  rm -f .omb/state/swarm.db
  rm -f .omb/state/swarm.db-wal
  rm -f .omb/state/swarm.db-shm
  rm -f .omb/state/swarm-active.marker
  rm -f .omb/state/swarm-tasks.db
  rm -f .omb/state/ultrapilot-state.json
  rm -f .omb/state/ultrapilot-ownership.json
  rm -f .omb/state/pipeline-state.json
  rm -f .omb/state/plan-consensus.json
  rm -f .omb/state/ralplan-state.json
  rm -f .omb/state/boulder.json
  rm -f .omb/state/hud-state.json
  rm -f .omb/state/subagent-tracking.json
  rm -f .omb/state/subagent-tracker.lock
  rm -f .omb/state/rate-limit-daemon.pid
  rm -f .omb/state/rate-limit-daemon.log
  rm -rf .omb/state/checkpoints/

  # Stop rate-limit daemon if running
  if [[ -f .omb/state/rate-limit-daemon.pid ]]; then
    kill "$(cat .omb/state/rate-limit-daemon.pid)" 2>/dev/null || true
    rm -f .omb/state/rate-limit-daemon.pid
  fi

  echo "All OMB modes cleared. You are free to start fresh."
  exit 0
fi
```
...
if [[ -f .omb/state/autopilot-state.json ]]; then
  ...
  if [[ -f .omb/state/ralph-state.json ]]; then
    RALPH_STATE=$(cat .omb/state/ralph-state.json)
    LINKED_UW=$(echo "$RALPH_STATE" | jq -r '.linked_ultrawork // false')

    # Clean linked ultrawork first
    if [[ "$LINKED_UW" == "true" ]] && [[ -f .omb/state/ultrawork-state.json ]]; then
      rm -f .omb/state/ultrawork-state.json
  ...
  CURRENT_STATE=$(cat .omb/state/autopilot-state.json)
  CURRENT_PHASE=$(echo "$CURRENT_STATE" | jq -r '.phase // "unknown"')
  echo "$CURRENT_STATE" | jq '.active = false' > .omb/state/autopilot-state.json

  echo "Autopilot cancelled at phase: $CURRENT_PHASE. Progress preserved for resume."
  echo "Run /oh-my-black:autopilot to resume."
fi
```
...
if [[ -f .omb/state/ralph-state.json ]]; then
  ...
  # Clean linked ultrawork first
  if [[ "$LINKED_UW" == "true" ]] && [[ -f .omb/state/ultrawork-state.json ]]; then
    UW_STATE=$(cat .omb/state/ultrawork-state.json)
...
if [[ -f .omb/state/ultrawork-state.json ]]; then
  ...
  if [[ "$LINKED" == "true" ]]; then
    echo "Ultrawork is linked to Ralph. Use /oh-my-black:cancel to cancel both."
    exit 1
  fi
...
```bash
echo "No active OMB modes detected."
echo ""
echo "Checked for:"
echo "  - Autopilot (.omb/state/autopilot-state.json)"
echo "  - Ralph (.omb/state/ralph-state.json)"
echo "  - Ultrawork (.omb/state/ultrawork-state.json)"
echo "  - UltraQA (.omb/state/ultraqa-state.json)"
echo ""
...
if [[ -f .omb/state/autopilot-state.json ]]; then
  AUTOPILOT_STATE=$(cat .omb/state/autopilot-state.json)
  ...
  if [[ "$AUTOPILOT_ACTIVE" == "true" ]]; then
    ...
    # Clean up ultraqa if active
    if [[ -f .omb/state/ultraqa-state.json ]]; then
      ULTRAQA_STATE=$(cat .omb/state/ultraqa-state.json)
    ...
    echo "$AUTOPILOT_STATE" | jq '.active = false' > .omb/state/autopilot-state.json
    ...
    echo "Progress preserved for resume. Run /oh-my-black:autopilot to continue."
...
if [[ -f .omb/state/ralph-state.json ]]; then
  ...
  if [[ "$RALPH_ACTIVE" == "true" ]]; then
    ...
    if [[ "$LINKED_ECO" == "true" ]] && [[ -f .omb/state/ecomode-state.json ]]; then
      ECO_STATE=$(cat .omb/state/ecomode-state.json)
...
if [[ -f "$SWARM_DB" ]]; then
  ...
  if command -v sqlite3 &>/dev/null; then
    ...
    MARKER_FILE=".omb/state/swarm-active.marker"
...
if [[ -f .omb/state/ultrapilot-state.json ]]; then
  ULTRAPILOT_STATE=$(cat .omb/state/ultrapilot-state.json)
...
if [[ -f .omb/state/pipeline-state.json ]]; then
  PIPELINE_STATE=$(cat .omb/state/pipeline-state.json)
...
if [[ "$CANCELLED_ANYTHING" == "false" ]]; then
  ...
  echo "  - Ralph (.omb/state/ralph-state.json)"
  echo "  - Ultrawork (.omb/state/ultrawork-state.json)"
  echo "  - Ecomode (.omb/state/ecomode-state.json)"
  echo "  - UltraQA (.omb/state/ultraqa-state.json)"
  echo "  - Swarm (.omb/state/swarm.db)"
  echo "  - Ultrapilot (.omb/state/ultrapilot-state.json)"
  echo "  - Pipeline (.omb/state/pipeline-state.json)"
  echo "  - Plan Consensus (.omb/state/plan-consensus.json)"
  echo ""
  echo "Use --force to clear all state files anyway."
fi
```
...
| Force | "All OMB modes cleared. You are free to start fresh." |
| None | "No active OMB modes detected." |

## What Gets Preserved

| Mode | State Preserved | Resume Command |
|------|-----------------|----------------|
| Autopilot | Yes (phase, files, spec, plan, verdicts) | `/oh-my-black:autopilot` |
| Ralph | No | N/A |
| Ultrawork | No | N/A |
| UltraQA | No | N/A |
| Swarm | No | N/A |
